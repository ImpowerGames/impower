...
The automatic `native-codex-jsonl` route is verified on Windows with executable version `0.154.0-alpha.6.2`. Its argument grammar is `exec --model <caller-model> -c 'model_reasoning_effort="<caller-effort>"' -c 'approval_policy="never"' --sandbox workspace-write -c sandbox_workspace_write.network_access=true -c 'model_provider="openai"' -c 'sandbox_workspace_write.writable_roots=[]' -c sandbox_workspace_write.exclude_tmpdir_env_var=true -c sandbox_workspace_write.exclude_slash_tmp=true --cd <private-review-directory> --skip-git-repo-check --ignore-user-config --ignore-rules --strict-config --json --disable multi_agent --disable multi_agent_v2 --output-last-message <fresh-private-report> -`. These are argument-array entries, not shell interpolation. Declare `permissions` as `{ "sandbox": "workspace-write", "approvalPolicy": "never", "networkAccess": true, "cwd": "<private-review-directory>", "artifactWrites": "handoff-directory" }`; use only when private artifacts and GitHub report posting are authorized. Each independent reviewer needs a disjoint private directory and fresh report path, excluding repository and supervisor state. The launcher adds only a fresh completion directory with `--add-dir`, separated from protected native stdout/stderr evidence. Alternate roots, profiles, config overrides and bypasses fail before launch.

Reviewer processes receive no originating host session credentials/routing. The Codex reviewer receives a fresh owner-protected `CODEX_HOME` containing only authorized authentication, without inherited configuration, rules, plugins or endpoint overrides. Native process stdout/stderr and actual launch arguments remain coordinator evidence; completion JSON is reviewer-authored. The coordinated live acceptance must verify actual `exec` sandbox enforcement, ordinary Git/report access and private-home compatibility. A standalone sandbox ACL probe is not equivalent evidence.

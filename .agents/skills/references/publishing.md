...
Every issue and pull request follows a template under `.github/`. Use `file-bug` to reproduce and file a defect, `file-feature` to settle design decisions and file a Feature, and [file-task](../file-task/SKILL.md) to file bounded maintenance or follow-up work without a mandatory interview. `resolve-issue` implements a specified ticket using regression, live verification and review at the applicable steps. GitHub only fills a template in for someone using the web form, so when you file from the command line (`gh api`, `gh pr create`) read the template file yourself and produce a body with the same headings in the same order:

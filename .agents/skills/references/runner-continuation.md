...
The caller-authored JSON plan needs `threadId`, `turnId`, `continuationId`, `destinationCwd`, `worktree`, `head` (full SHA), and an absolute `journal` outside both the experiment worktree and the originating checkout, their shared Git directories and registered existing worktrees. The destination is the originating task's directory; the experiment worktree may be different. `destinationCwd` must match the host-reported spelling after `path.resolve` on each value. Native alias normalization protects containment but does not broaden destination identity, so an equivalent filesystem alias can still be refused. Run from the originating task's inherited environment. The journal uses exclusive creation and flushed append records. Keep the source files fixed while the probe runs. Existing journals are refused, including blocked experiments; inspect the retained evidence before authoring a new experiment. Failure after submission intent remains uncertain and is never retried.

Both plan paths must be directories in Git repositories whose checkout paths can be verified. A file-valued path or non-repository destination is refused with its path named. Repository lookup and the frozen-head check remove inherited `GIT_DIR`, `GIT_WORK_TREE` and `GIT_COMMON_DIR` from the Git subprocess environment, retaining discovery ceilings and other configuration. Layouts where Git reports shared storage itself as a checkout, including the tested separate-Git-directory and bare-repository-with-linked-worktrees layouts, are deliberately unsupported and refused before any journal or host call. The storage lookup is defensive in addition to the reported-worktree exclusions; in the verified ordinary layout those protections overlap. No independently load-bearing claim is made for the redundant common-directory exclusion.

Containment uses native filesystem canonicalization consistently for the journal's existing parent, plan directories, common storage and reported checkouts. On Windows, this closes the demonstrated NTFS short-name alias and extended-length spelling cases. Only directories that canonicalize to drive-letter paths are supported; UNC paths and native network-directory results are refused rather than assumed equivalent to a local checkout. No SUBST or mapped network drives were present in the read-only local inventory, so equivalence across those drive aliases is not a demonstrated capability. Existing journal files, including hardlinks, remain refused by exclusive creation. Windows final-file symlinks and concurrent path replacement remain untested; these checks do not establish safety against adversarial filesystem changes. `GIT_WORK_TREE` removal is defensive alongside the two independently pinned routing variables; it is not claimed independently load-bearing for the current Git queries.
...
An empty or partial acceptance sidecar after a write failure is not acceptance evidence. Its exclusive filename still prevents automatic overwrite; preserve it and the original journal for manual inspection. Delivery remains uncertain, and no resend is authorized. This scratch command does not implement recovery of a partially written sidecar.
...
- The host send schema has no documented idempotency or conditional expected-turn argument. Inspection and submission are not atomic; a new turn in that interval still requires serialization and receiving-side ownership checks. Native cancellation in that interval need not suppress later queued work under the revised contract.
...
These identifiers came from the two session transcripts, including the actual tool receipt, not just the sender's narrative. A distinct host turn ID was not exposed in those rows; message UUIDs are not presented as turn IDs. The terminal and transcript showed Opus 5/high for the receiver. The sender usage also reported a small CLI-internal Haiku 4.5 auxiliary call; no task model fallback was requested. The later authenticated probe below establishes the selected task model/effort and permissions without a sending model.
...
## Later receiver-veto probe and selected host

The maintainer subsequently selected Claude CLI 2.1.270 or later; no global upgrade was performed. This probe predates the revised cancellation scope and investigated the former requirement that native Stop/Escape cancel automatic continuation. A separate private 2.1.270 interactive session tested the documented [UserPromptSubmit veto](https://code.claude.com/docs/en/hooks#userpromptsubmit) with the same explicit Opus 5/high/dontAsk route and no built-in tools. The command hook intercepted an authenticated inbox message and blocked its fixed marker before a model response. However, a second marker deliberately delayed that veto by fifteen seconds against its ten-second hook timeout. The terminal reported that the hook timed out and discarded its output, then the marker reached Claude: native turn `2c0ab69c-1697-43a7-997d-9d3562704cf1` in the same session `2dab7b3e-7d7e-4bdc-944f-87d6aefab0cd`, at `2026-09-14T03:30:16.989Z`. This agrees with the documented command-hook timeout behavior and rules out that hook alone as a failure-safe cancellation gate. SDK callback hooks have different timeout behavior, but attachment of that control to this already-running interactive host has not been established.

Receiver PID 37700 exited 0 through `/exit`; the SessionEnd record at `2026-09-14T03:30:35.698Z` reports `prompt_input_exit`. Its PID and observed hook PIDs were subsequently absent. Private evidence is in the temporary `impower-546-veto` directory (`hooks.jsonl`, submission records and `result.md`), its sibling hook/sender/settings files, and that session's transcript. The private endpoint file contains authentication material and must not be published. This was a bounded harmless admission-control experiment, not a reviewer run or a production adapter.

An additional process-local experimental function-hook probe on 2.1.270 blocked normal input, exceptions and asynchronous timeouts through an explicit catch. A worker watchdog failure unloaded the module and allowed both the failing and subsequent guarded message to run in the same session (`1ef4f3c1-a08e-423e-bbab-f93f41ed659d`, native turns `890cf77e-2b61-400a-9303-940a5fb97b9c` and `7aa916e5-d404-43d0-af6b-c2121be27764`). Both original probe processes exited normally and were confirmed absent. This rules out the module alone for the former stronger failure-safe gate; it does not block the revised active-turn contract. Safe private summary: `impower-546-cancel-optin/result.md` in the originating user's temporary directory.
...
Static inspection of pinned acpx 0.15.1 confirms that its `cancel` command targets the active prompt through owner IPC and ACP `session/cancel`; a startup cancellation is latched until that prompt is active. Pending prompts remain queued, the latch clears with the turn, and an idle request does not cancel future work. A successful request receipt is not confirmation of stopped execution. Signal paths differ: signalling the queue owner shuts it down and discards pending tasks, while a queue client's disconnection does not itself remove an accepted task. Do not claim identical queue effects for every Ctrl+C path. No live ACP cancellation run was performed.

...
Each new editor or extension element gets a block under Proposed solution that the experience review checks mechanically, with the four items the interview settled. Write it as plain lines or inside a fence, with plain or bold labels, and put a label's text on the label line or on bullets nested under it:

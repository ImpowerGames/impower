...
Stop after any completed round when [readiness](adjudication.md)'s gates pass; neither three rounds nor consecutive clean rounds are required. The autonomous cap is three rounds: initial review, correction review if needed, and a final correction review if needed. A failed attempt or pending reviewer on the same frozen head remains in its original round, with an incremented attempt for retries. Finish all required reviewers before treating the round as complete.

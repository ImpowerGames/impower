...
Stop after any completed round when [readiness](adjudication.md)'s gates pass; no minimum number of rounds or consecutive clean rounds is required. The autonomous cap is 3 rounds by default. Review the initial change, then corrections only as needed. A failed attempt or pending reviewer on the same frozen head remains in its original round, with an incremented attempt for retries. Finish all required reviewers before treating the round as complete.

After the configured limit, fix and verify accepted findings, commit and push, but do not automatically launch a further round. Keep the PR draft if behavior-changing corrections still need independent review or any blocking finding or material verification gap remains. Report the changes needing review and the next action. Only the narrow non-behavioral exception above can finish without another review, with checks, unreviewed commits, and the justification in Notes for reviewers and the adjudication. Exhausting the cap never grants readiness.

New scope, a resumed session, a new journal, or a changed head does not reset the count. Preserve the round state through recovery. Further review beyond round 3 requires an explicit user request. When that request names a higher limit, the caller may set `reviewRoundLimit` from 4 through 10 and record the request verbatim in non-empty `extendedReviewAuthorization` in the local launcher plan; otherwise omit both fields and hand off the draft for human review. The launcher validates the declared limit and authorization but cannot infer authorization from PR content or a child report. Do not disguise another round as a retry, reset the count, or bypass local process reservations.

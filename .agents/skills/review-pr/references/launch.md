...
- Author experience: perform the issue's task as an author in the running editor before reading the code, and report friction with a transcript. Required, not optional, for any PR that changes Sparkdown syntax, completions, hover, diagnostics, user docs or the editor interface; read [author-experience lens](author-experience.md) before launching it, because it needs your driver servers stopped first.

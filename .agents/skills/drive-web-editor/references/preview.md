...
- `scrub`: the protocol selection that drove the scrub. The compatibility field `clicked: true` with a matching `cursorLine` confirms selection. `previousPosition` and `position` expose the player's selected source position before and after the request; they do not prove that rendering finished. `clicked: false` carries a `reason` instead, and that is a real failure worth reading.

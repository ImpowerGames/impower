...
It measures the story engine apart from the compiler, the route search's bookkeeping and the transport, and sets it beside a stepping loop that runs the binary program buffer directly (#664). It needs the workspace install; `scripts/bench/engine-bench.test.mjs` runs it once on the fixture when that install is present. Every candidate runs in a process of its own and reports min, median and max over `--samples` (default 12) after `--warmup` (default 4). `--mode` takes any of the modes, comma separated, and defaults to all of them:

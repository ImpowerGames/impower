...
It needs the workspace install; `scripts/bench/preview-fixture.test.mjs` runs it once on the fixture when that install is present. It bundles `scripts/bench/previewBench.ts` with esbuild and runs each mode (`--mode preview`, `edit` or `both`, the default) in a process of its own with a 4 GB heap; a large project does not fit under vitest's 1024 MB limit. It replays `packages/spark-web-player/src/main/workers/workspace.worker.ts` with no browser, as the player ships: the compiler configured as the player's worker configures it, with no compiled story emitted, the worker's `Game` and `searchRouteTo`, and the display of each result from that game. Replacements default to every image file whose name starts like the identifier around `--word`; `--options a,b,c` overrides them.

The display runs the worker's steps (`displayPreviewFrom`): the game that searched the route loads its own checkpoint, connects to a sink standing in for the page and previews. The sink clones every message through `v8.serialize` and `v8.deserialize`, as a post to the page does, and answers requests the way the page does (`ui/write-text` and `ui/write-image` with their target, `ui/animate` with every effect, `assets/load` with everything loaded, `audio/load` with an `outputLatency` of 0, `world/load` with its params).

It prints, as min, median and max per sample: wall-clock times for the worker (compile, game update and route together), the game's `load`, its connect and preview, the message clone inside them, and the total without the DOM, which counts the worker, the `load` and the clone and leaves out the connect and preview themselves; the compiler's and route search's own `profile()` phases (`game/planRoute`, `ink/compile`, `incrementalParse`, `populateLocations`, `ink/flowShapes`, `game/setStartFrom` and the rest); and the display's messages: their count, the kilobytes cloned to the page and back, the largest message and the kilobytes by method, the requests the sink does not model, and a line that names any program, checkpoint or path locations found in what was cloned. It also lists the engine calls made on the route game before each display, that game's preview state before and after, and the messages it sent while connected but outside a display. `--json <file>` writes each mode's full report to `<file>.<mode>.json`.

The compiler emits no compiled story, so it certifies an edit's changes as confined from a walk of every flow timed as `ink/flowShapes`, and the route search resumes its last route instead of searching the scene from the top.
...
`--cpu-prof <dir>` also writes a V8 CPU profile of each process into `<dir>` (`preview.cpuprofile`, `edit.cpuprofile`), with the bundle's source map beside it, for `profile-shares.mjs` below; that bundle keeps its function names and runs slower, so take times from a run without the flag. Beside each profile it writes a `.gaps.json` of the same name, the unattributed stretches of every measured sample on the profile's clock, and `--gaps` charges them to functions:

...
- The runner found no test: `--files` named the test file itself, so the revert removed it. List only the changed sources. The classifier recognizes no-test wording only on standalone runner-diagnostic lines; a passing test name that mentions "no tests found" cannot override a genuine assertion failure elsewhere in the output. When test output itself prints the exact phrase on a standalone line, a positive Vitest `Tests` count establishes the assertion; without that count, mixed no-test and assertion text remains unverified rather than guessed.

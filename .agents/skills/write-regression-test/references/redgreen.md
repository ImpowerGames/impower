...
- The runner found no test: `--files` named the test file itself, so the revert removed it. List only the changed sources. The classifier recognizes no-test wording only on standalone runner-diagnostic lines; a passing test name that mentions "no tests found" cannot override a genuine assertion failure elsewhere in the output. Test and wrapper stdout is untrusted and can imitate runner banners, assertions, and summaries, so any standalone no-test diagnostic mixed with assertion evidence remains unverified unless the driver gains process-level attribution.
...
- The red or green run never started (`outcome: "not run"`): the test-suite runner printed `test-suite: not run:` because it could not take the machine-wide reservation within `--wait`. The run says nothing about the defect or the fix; run redgreen again once the other run finishes.

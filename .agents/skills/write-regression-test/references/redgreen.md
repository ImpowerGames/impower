...
- The runner found no test: `--files` named the test file itself, so the revert removed it. List only the changed sources. The classifier recognizes no-test wording only on standalone runner-diagnostic lines; a passing test name that mentions "no tests found" cannot override a genuine assertion failure elsewhere in the output. When test output itself prints the exact phrase on a standalone line, a Vitest banner and paired positive `Test Files` / `Tests` summary in the same invocation establish the assertion; a later invocation cannot lend its summary, and mixed evidence without that provenance remains unverified rather than guessed.

...
Run the test file under work with the single-file command in [Running vitest safely](vitest.md). Package suites are not run locally: `.github/workflows/test-suite.yml` runs on every pull request, and its package jobs run one package per job when the change touches `packages/`, `impower-dev/`, `definitions/` or the root package files. On any other change the package jobs report as skipped, which satisfies the required check; say so in the PR rather than citing a run that did not execute. The run for the pushed head is the suite evidence for the PR. Read its conclusion with `gh pr checks` or `gh run view`; a failure there is yours to fix unless you confirm it also fails on `origin/main`. The same skip-when-irrelevant gating, in `.github/workflows/changed-paths.yml`, fronts the typecheck, standalone-checks and notifier-tests workflows so every required check always reports. When you need a local package result the workflow cannot give, such as a baseline on a base commit, use `node scripts/test-suite.mjs start <package-directory>` under the caps in that reference and retain its run directory.
...
A project your change reaches through an import is checked when that project runs, not when yours does, so widen to the whole gate before you push. CI runs the same command on any pull request that touches code, a `tsconfig`, or a `package.json`; on a branch that changes only docs the `typecheck` job in `.github/workflows/typecheck.yml` reports as skipped. Where it does run, a type error blocks the merge, so a clean local run is worth something and a red one is a real failure rather than noise to route around.

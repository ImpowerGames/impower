...
Locally, check the projects you touched, plus any project you know imports the code you changed, since an importing project is checked when it runs, not when yours does. The whole gate is the `typecheck` workflow's: `.github/workflows/typecheck.yml` runs the unfiltered command on any pull request that touches code, a `tsconfig`, or a `package.json`, and its result on the pushed head is the whole-gate evidence for the PR. On a branch that changes only docs its `tsc` job is skipped and the required `typecheck` gate job passes. A type error there blocks the merge and is yours to fix, not noise to route around. A repository hook refuses the unfiltered `npm run typecheck` at the root and `scripts/typecheck.mjs` with no filter; a package's own `npm run typecheck` is already filtered to that package and is allowed.

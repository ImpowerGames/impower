...
A direct Vitest call locally names the test files under work. A repository hook (`.agents/hooks/local-test-hook.mjs`) refuses a call with no file argument, a directory, a glob, a name that is not an existing test file, and a package `npm test`, `npm run test`, `pnpm test` or `yarn test`; it allows the command below and `node scripts/test-suite.mjs`. It reads command text statically, so a command built from a variable or an alias, or handed to another program as a string, is outside its coverage and this paragraph is the rule. The snapshot/restoration driver still accepts a direct single-file command. Check existing Vitest processes and the suite reservation first, then keep the heap and fork caps:

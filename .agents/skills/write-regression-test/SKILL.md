...
- The red run's output is unrecognized or mixes assertion and shell diagnostics (`reason: "unknown"`, or no output at all). A test printing its child's shell error and a broken command chain can produce identical output and status, so neither is automatically accepted as proof. Read the full run yourself and explain the ticket's actual failing assertion in the PR; a broken invocation, config error, worker crash or truncated run proves nothing. For a POSIX shell, status 126/127 with no assertion evidence is treated as an execution failure independently of locale; with assertions it is ambiguous, since programs can choose those statuses too. This convention is not applied to cmd. Caller environment values are preserved. Other shell wording is recognized in English; unrecognized localized output still requires inspection.

...
export function decide(request) {
  ...
  if (request.kind === "shell") return typedIssue(request.command, request.shell) ?? sharedStash(request.command, request.shell) ?? (request.shell === "bash" ? null : dotNetRelativePathReason(request.command));
  return null;
}

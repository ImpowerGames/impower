...
const getSuffixedVariants = (word: string): string[] => {
  if (word.endsWith("_")) {
    return [];
  }
...
const getSuffixed = (term: string): string[] => {
  ...
  if (!term.includes(" ")) {
    return getSuffixedVariants(term);
  }
  if (!term.includes("~")) {
    return [];
  }
  ...
  words.forEach((word) => {
    if (word.endsWith("~")) {
      ...
      getSuffixedVariants(cleanedWord).forEach((alt) => {
...
const getPrefixedVariants = (word: string): string[] => {
...
const getPrefixed = (term: string): string[] => {
  term = term.toLowerCase();
  if (!term.includes(" ")) {
    return getPrefixedVariants(term);
  }
  if (!term.includes("~")) {
    return [];
  }
  ...
  words.forEach((word) => {
    if (word.startsWith("~")) {
      ...
      getPrefixedVariants(cleanedWord).forEach((alt) => {
        const regex = new RegExp(word, "g");
        alternativeTerms.add(term.replace(regex, alt));
      });
    }
...
export const getTermVariants = (term: string): string[] => {

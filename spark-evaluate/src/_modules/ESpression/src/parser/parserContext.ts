...
export class ParserContext {
  ...
  tokens: { type: string; content: string; from: number; to: number }[] = [];
   ...
   */
  parseNext(jump: string | number, includeSpace?: boolean): INode {
    ...
    try {
      ...
      res = rule.pre(this) || this.parseNext(1, includeSpace);
      if (!includeSpace) {
        this.gbSp();
      }
      ...
      res.from = from;
      res.to = to;
      const content = this.e.slice(from, to);
      const type = res.type;
      if (
        !this.tokens.some(
          (t) =>
  ...
  private moveRule(jump: string | number = 0): IRule {
    if (typeof jump === "string") {
      if (!(jump in this.rules)) {
        throw new Error(`No registered label ${jump}`);
      }
      this.branch = jump;
      this.level = 0;
    } else if (jump === 1) {
  ...
  parseMulti(
    ...
    do {
      ...
      try {
        const jump = Array.isArray(jumps)
          ? jumps[index] || jumps[jumps.length - 1] || 1
          : jumps;
        node = this.parseNext(jump, c.includeSpace);
        ...
        if (e instanceof SyntaxError && (<ParseError>e).noMatch) {
          ...
          if (!sep || sep === true) {
            ...
            if (node) {
              node.from = curPos;
              node.to = this.eof() && !this.lt ? this.ch : this.i;
            }
          }
        } else throw e;
      }

...
export class UnaryOperatorRule extends BaseRule<IConfUnaryRule> {
  ...
  constructor(public override config: IConfUnaryRule) {
    super();
  ...
  override post(ctx: ParserContext, bubbledNode: INode): INode {
    ...
    const o = c[op];
    if (!o) {
      return ctx.err(`Invalid operator: ${op}`);
    }

    if (o.types && o.types!.indexOf(bubbledNode.type) < 0)

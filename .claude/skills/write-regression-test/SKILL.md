...
`git ls-files` sidesteps both: the checks are tracked, and everything `find` picks up by mistake is ignored or untracked. Count the `---` lines against what `git ls-files '.claude/**/*.test.*'` returns: the loops print one per check they run, eleven: two under `.claude/hooks/` (one shell, one Node), six Node checks under `.claude/skills/drive-web-editor/`, one Node check under `.claude/skills/clean-worktrees/`, and one shell check each under `.claude/skills/resolve-issue/` and `.claude/skills/review-pr/`. Stage a new check before running the loop, because `git ls-files` sees only what is tracked.

The checks need no `node_modules`: the driver imports `playwright` only inside the commands that launch a browser, so a worktree that skipped `npm install` still runs all eleven.

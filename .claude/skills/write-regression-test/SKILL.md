...
Derive the tracked check total with `git ls-files '.claude/**/*.test.*' | wc -l` and keep `.github/workflows/hook-tests.yml`'s discovery floor at that total. The checks need no `node_modules`: the driver imports `playwright` only inside the commands that launch a browser, so a worktree that skipped `npm install` still runs every one of them.

...
The two loops take about two minutes here (1m37s at `bfb1d836c`, most of it Node startup for the `.mjs` checks) and longer on a loaded machine, because they spawn many small processes; give them a ten-minute timeout and read the result rather than concluding a hang. `reviewer-model-values.test.sh` is out of the loop because a full run can outlast the loop's ten-minute timeout (3m58s and 17m44s on two runs on this machine, 46 assertions passing). The quick pass above covers a change to a reviewer definition or to the review-pr skill; the review-pr skill, which owns the check, says when the full run is needed, how to run it and what its output looks like while it runs.

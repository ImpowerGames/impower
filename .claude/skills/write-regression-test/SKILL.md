...
- The command names vitest, or its output shows vitest's own run banner, and the red run failed with what reads as a real assertion but no `Test Files`/`Tests` summary line could be parsed from it (a reporter or version this parser does not know). Read `red.tail` for the failure and quote it directly in the PR.
...
This in-place path runs no `redgreen` command, so its recovery is the aside copy you kept, not a `redgreen` snapshot directory. Keep the disable-and-red-run half under the command tool's own timeout, separate from the restore: that shrinks the window in which a hard kill can leave the file disabled, though it does not remove it, since a timeout during the first half can still leave the branch disabled. If it does, compare the aside copy's content hash against the working file to tell whether the restore already happened, and copy the aside copy back by hand if it did not.

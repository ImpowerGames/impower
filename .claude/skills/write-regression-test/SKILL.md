...
Then run the standalone checks under `.claude/`, from the repo root: `git ls-files` pathspecs are relative to the current directory, and from anywhere else the loops below run nothing and exit 0. `.github/workflows/hook-tests.yml` runs this same loop over every check on any pull request that touches `.claude/`, so a check you break is caught before the merge; run it locally anyway, because that job runs on `ubuntu-latest` and a case gated to Windows prints `SKIP:` there and proves nothing. Each check pins a footgun that has already cost a session. There are two kinds, shell checks over hooks and skill prose, and `.mjs` checks over the hooks, the browser-free functions in the two drivers, the clean-worktrees script and the size of the skills' own Gotchas and Troubleshooting lists, one loop each:
...
`git ls-files` sidesteps both: the checks are tracked, and everything `find` picks up by mistake is ignored or untracked. Count the `---` lines against `git ls-files '.claude/**/*.test.*' | wc -l`: the loops print one per check they run, so a shorter count means a check was skipped. Stage a new check before running the loop, because `git ls-files` sees only what is tracked — and `.gitignore` excludes `.claude/*`, re-including named subdirectories one at a time, so a check under a new one needs its own `!` line there before `git add` will stage it, the loops will list it, or CI will run it. Run the loops with the machine otherwise idle: a check that holds a time bound (`typed-issue-hook.test.mjs` decides 32,000 unbalanced groups in under 2 s) misses it by milliseconds under a concurrent browser run, and a miss it does not repeat alone is the load, not the check.

The tracked check total is derived with `git ls-files '.claude/**/*.test.*' | wc -l`; it is 17 with the triage-skill-feedback check, and `.github/workflows/hook-tests.yml` holds its discovery floor to that total. The checks need no `node_modules`: the driver imports `playwright` only inside the commands that launch a browser, so a worktree that skipped `npm install` still runs every one of them.

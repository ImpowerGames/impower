...
`find`, not a `**` glob: without `shopt -s globstar` bash reads `**` as `*`, so `.claude/**/*.test.sh` silently matches only the check under `.claude/hooks/` and skips the skill's own, and the loop reports success having run half the checks. There are two shell checks (`.claude/hooks/`, `.claude/skills/resolve-issue/`) and four Node ones, all under `.claude/skills/resolve-issue/`; the loop should print six `---` lines.

One of the six is not quick. `reviewer-model-values.test.sh` re-runs itself once per control case (about thirty-five) and spawns `awk`/`grep`/`sed` per field, so on the reference machine it takes about eighteen minutes idle (measured 17m44s, 46 assertions passing) and longer under load, and prints nothing until its base section is done. Run it in the background with a long timeout, or `REVIEWER_CHECK_INNER=1 bash …` for the quick pass over the real files only (the controls matter when that script or the reviewer definitions change). The other five finish in seconds. A shell check that hangs rather than failing is usually the machine, not your change: they spawn many small processes and a running dev-server build starves them. Re-run once the build finishes before believing a timeout.

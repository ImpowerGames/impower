...
Verified output shape (steps first, then the read-back; `url`, `startedOn`, the run's top-level `editorSettled`, `ui.tabs` and `consoleErrors` are also present and omitted here):

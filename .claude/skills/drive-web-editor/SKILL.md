...
Two more wait on the tickets that own the mechanism, and go when it lands:

- `down` then `up` does not put a changed service worker under test: the dev server rebuilds `sw.js` on request, but the persistent profile keeps the worker it installed, so a before/after of anything inside it (`impower-dev/src/workers/sw.ts` and the modules it imports, so `/file:/`, `?thumb=` and `?filters=`) measures the same build twice and reports no difference. Until #501 adds `--fresh-sw`, unregister every worker and reload from a script on the exported helpers, and confirm which build answered by having the worker read its own script for a phrase only one version has; worker-side `console.warn` never reaches the page's capture, so hook it inside the worker with the same `evaluate`.
- A `Cache.put` the filtered or thumbnail generator cannot complete costs the memo, not the art (#477): the generator warns and serves the variant it already built, so a timing run that expects a cache hit on repeat and does not get one is reading this rather than a regression. The warning is worker-side, so #501's report field is what will surface it; until then, hook the worker's console as above.

...
- `program.loaded: false` (with a `programWarning`): the player had not loaded a program within 90 s of the game mounting, read from the toolbar's launch-state icon, which the player sets when a program reaches it. `program.errors` is the error count the editor's status bar shows: above zero, the script does not compile, so the player was never given a program and the preview is the picture of a script that does not compile, which is evidence only when that is the bug; zero, the harness was not ready (the first compile was still running), the scrub that followed was sent to a player that was not listening, and the preview is not evidence. `program.ms` is how long the wait took, whether or not it succeeded. `program.loaded: null` with a `reason` means nothing was waited for: the preview is not observable (cross-origin mode) or the game never mounted.
...
| Step                     | Does                                                                                                                                                  |
| ------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
| `--sd <file.sd>`         | load the script into OPFS and reload, as `verify` does; then waits for the editor view to settle (`editorSettled`) and, where the game preview is observable, for the program to reach the player (`programLoaded`; `programNote` says why when it did not within 90 s, or that the game never mounted, where `programLoaded` is `null`) and for the first compile (`previewSettled`; `previewNote` says when it is not observable: cross-origin mode, or the preview showing the screenplay) |
| `--project <dir-or-zip>` | replace the OPFS project with every file of a directory or an exported zip and reload, as `verify --project` does (section 1); the step carries the `seed` report, and fails without reloading on its `reason` |
...
- A `--project` seed that stops part-way (a refused write, a refused removal, a Ctrl+C) leaves `local/.seeding` in storage and the previous project's entries under the files that landed; nothing is removed until every file has landed, so the previous project is never destroyed by a seed that fails. A plain `verify` or `ui` on that storage stops with an `error` naming the marker rather than driving a mix of two projects; re-run with `--project`.
...
| `error` names `local/.seeding` on a run without `--project`                | An earlier seed stopped part-way and storage holds a mix of two projects. Re-run with `--project`.                                                                    |

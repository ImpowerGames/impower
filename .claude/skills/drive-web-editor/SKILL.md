...
The driver must live inside the repo tree: Node resolves `playwright` relative to the script's directory, not the working directory, and a copy outside it says so rather than dying in a bare `ERR_MODULE_NOT_FOUND`. The same applies to any script of your own that imports the driver's exported helpers: put it under the worktree (and never `git add` it), not in the scratchpad.
...
`ui --hover 5:19` opens a hover with real pointer movement; `ui --complete 7:3=~fil` types real keystrokes at the requested position and waits for completion. Positions count lines and UTF-16 columns from one, matching the status bar. Both use the go-to panel and verify the caret before acting. Hover reports `present`, `text`, `imgSrc`, `imgRect`, `naturalWidth` and `naturalHeight`; completion reports `popupPresent`, `options`, `selected`, `infoPanelPresent` and the same image fields. `--shot-of hover hover.png` crops the hover; `--shot-of completion completion.png` includes the list and its info panel together. A missing surface fails the step and reports `serverResponse: "unobserved"`: the DOM alone cannot distinguish an empty language-server answer from a request that failed. These steps use the editor's existing controls; protocol-driven operations belong to #500.
...
`verify --fresh-sw` unregisters the origin's service workers and reloads before verification; `ui --fresh-sw` does so at that point in the step sequence, so put it before the actions whose worker activity matters. The report is `serviceWorker` on `verify` and on the corresponding `ui` step. It includes `refreshed`, `controlled`, `scriptURL`, and a SHA-256 of the installed worker's script read through Chromium's debugger. A temporary message listener confirms that the hashed worker answers through the page's controller, then is removed. A failure stops verification; no hash is inferred from the server's current file. Worker-side console `warnings` and `errors` are captured from the refreshed worker, including a filtered-image or thumbnail cache write that fails while serving the generated art. Each list retains up to 25 captured messages; an empty list does not prove a cache hit. Compare `sha256` between runs: changing worker code must change it, and restarting unchanged code must preserve it. OPFS and Cache Storage are retained.

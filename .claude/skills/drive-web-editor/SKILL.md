...
- `program.loaded: false` (with a `programWarning`): the player had not loaded a program within 90 s of the game mounting, read from the toolbar's launch-state icon, which the player sets when a program reaches it. The first compile was still running or the script does not compile; the scrub that followed was sent to a player that was not listening, and the preview is not evidence. `program.ms` is how long the wait took when it succeeded.
...
`--sd` writes one script, and a change or bug that involves assets (portraits, backdrops, audio, image previews, the asset inspector, anything a script reaches through a file) needs the whole project in the editor's storage. `--project <dir-or-zip>` on `verify` and `ui`, and `seed --project <dir-or-zip>` on its own, write every file of a project directory or an exported project zip under `local/<relative path>` in OPFS and reload as `--sd` does; with both flags the project goes in first and the script over its `main.sd`, so a repro script runs against the project's assets. The source is read where it lives and nothing is copied into the tree, so an absolute path outside the repository is the normal case: the Raffles & Bunny project the maintainer reproduces against is a private checkout outside this repository, and its path comes from the maintainer or from memory. The seed replaces the project the way the editor's own zip import does: every entry under `local/` whose name does not start with a dot is removed first, so a file from an earlier seed cannot stand in for one the source lacks, while dot entries (`.name`, `.trash`) are the editor's own and stay, and dot entries in the source are skipped. A zip is unpacked in Node with the library the editor's import uses, and an archive that wraps the project in one top-level folder is unwrapped. The report lands under `seed`:

```json
"seed": { "source": "C:\\...\\project", "project": "local", "kind": "directory", "files": 673, "bytes": 198445312, "batches": 23, "removed": ["assets", "main.sd", "scripts"], "failed": [], "mainSd": true, "ms": 29684 }
```

`files` and `bytes` are what was written and read back, not what was sent. A file that could not be written is in `failed` with its path and the storage's reason, `reason` says so, and the run fails on it: `verify` exits 1 with that `error` and takes no screenshot, and a `ui --project` step lands in `failed` without reloading. Audio is most of a game's bytes (134 MB of the Raffles & Bunny project's 187 MB), so a seed of the whole project takes about half a minute here; the pinned origin keeps it across `down`/`up`, so seed once and re-use it as with `--sd`. A project this size also compiles for several seconds after the reload, which is why `verify` waits for the program to reach the player before it scrubs (`program.loaded`, below).
...
| Step                     | Does                                                                                                                                                  |
| ------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
| `--sd <file.sd>`         | load the script into OPFS and reload, as `verify` does; then waits for the editor view to settle (`editorSettled`) and, where the game preview is observable, for the program to reach the player (`programLoaded`; `programNote` when it did not within 90 s) and for the first compile (`previewSettled`; `previewNote` says when it is not observable: cross-origin mode, or the preview showing the screenplay) |
| `--project <dir-or-zip>` | replace the OPFS project with every file of a directory or an exported zip and reload, as `verify --project` does (section 1); the step carries the `seed` report, and fails without reloading when any file did not land |

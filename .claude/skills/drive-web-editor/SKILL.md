...
The state file, `.state.json` beside the driver (gitignored), records the editor URL, the launcher pid, the mode and the ports. A tree launched from the driver's location under the resolve-issue skill left its state file there, and the driver reads that one while nothing sits beside it, so `status` ends its line with `state=<path>` to say which file it read. `up` reuses a recorded tree only while its record stands: the pid is alive and either the editor port is held (the editor binds it before the cold worker build) or the record is under fifteen minutes old; a live pid alone can belong to any process by now, so a record with less than that is stale and `up` replaces it. `down` stops a tree only through a record that stands and removes a stale one without signalling anything.
...
| `up` says `servers pid N are still starting` and then times out             | The record stood for the whole wait (pid alive, editor port held), so a tree is there and not answering. `down` stops that tree through the record; then `up`. A record that stops standing mid-wait does not reach this: `up` says `are gone; launching` and starts a fresh tree.        |

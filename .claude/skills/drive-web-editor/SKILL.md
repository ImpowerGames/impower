...
- `scrubCheck`: a hint about whether the scrub landed, never the gate; opening the PNG and looking is the primary check, always. Three outcomes, and the third — `inconclusive` — is a real answer rather than a soft failure:
...
`--sd` is only needed when the script changes: the pinned port keeps the same origin, so OPFS survives `down`/`up` and a plain `verify --line N --shot x.png` re-uses the script already loaded. Always pass `--sd` when switching between repro scripts; without it `verify` silently scrubs whatever script was loaded last, and the report does not say so.
...
- When timing a scrub, run one throwaway scrub on a line you will not measure and wait a minute before measuring. The driver refuses a click that would land on the position the caret already holds and says so (`clicked: false` with a reason naming the unchanged selection) rather than clicking and reporting a success that moved nothing, so the throwaway must target a line other than the one you are about to measure. This only helps inside one page session, kept open across both scrubs on a script built on the driver's exported helpers: the CLI's `verify` reloads the page on every invocation, so a throwaway scrub run through one `verify` call warms nothing for a measurement taken by the next. The minute is for the preview session itself: a scrub that lands while the first one after a reload is still mid-build races it and mounts a stacked or ahead-of-cursor preview instead of the one that scrub asked for (#456), which would corrupt a timing run far more than it corrupts a screenshot.
...
- A `--project` seed that stops part-way (a refused write, a refused removal, a Ctrl+C) leaves `local/.seeding` in storage and the previous project's entries under the files that landed; nothing is removed until every file has landed, a previous entry whose kind clashes with the source refuses the seed before any write, and `seed --clear --project` clears only once the source has been read and accepted, so the previous project is never destroyed by a seed that fails. On that storage a plain `verify` stops with an `error` naming the marker, and `ui` refuses every step but `--probe` until a `--project` step has seeded, rather than driving a mix of two projects; re-run with `--project`, or empty the project with `seed --clear`. A seed the storage refuses for space needs the clear first: the previous project and the seed share the quota until the prune.
...
- On a cold origin the first `selectionSet` is dropped because the player worker is not listening yet. The driver waits for the program to reach the player (the toolbar's launch-state icon) and then for the first compile to settle before scrubbing; the settle alone is not enough, because an empty game reads as settled at once, and on a project whose compile takes seconds the scrub then lands on nothing and the preview stays black. Without that wait you get beat 1–2, or nothing, no matter what line you ask for.
...
- Typing a bracketed command into the editor with `page.keyboard.type` gives you more brackets than you typed. The editor auto-closes `[[`, so `type("[[show portrait bunny]]")` leaves a stray `]]` behind the caret, and the next Enter carries it onto the following line. A trailing `>` then sits mid-line instead of at the end, so the break marker does nothing and two beats silently become one — which reads as a preview bug rather than as a typo, and has already produced a ticket whose second reported symptom was the artifact. Type the opener and let the editor supply the closers (`type("[[show portrait bunny")` then `press("End")`), and read the document back with `documentLines` before trusting anything the run reports. The driver's `--sd` path writes the file directly and is unaffected; this bites only a script of your own that types.
- `page.goto` and `page.reload` need an explicit `timeout` in a script of your own. Playwright's default is 30 s and a cold editor load here takes longer, so the script dies with `Timeout 30000ms exceeded` on a server that is perfectly healthy. The driver's own navigations pass `{ waitUntil: "domcontentloaded", timeout: 120_000 }`; copy that rather than the bare call.
...
- `down` then `up` does not put a changed service worker under test. The dev server rebuilds `sw.js` from source on request, so the served bytes really do change, but the persistent profile keeps the worker it already installed until the browser re-fetches and compares that script — and it may not, within a run. A before/after of anything running inside the worker (`impower-dev/src/workers/sw.ts` and the shared modules it imports, so the `/file:/` asset path, `?thumb=` and `?filters=`) then measures the same build twice and reports no difference, which reads as "the bug is not real". Unregister every worker and reload to swap it, from a script on the exported helpers: `page.evaluate(async () => { for (const r of await navigator.serviceWorker.getRegistrations()) await r.unregister(); })`, then `page.reload(...)`, then wait for `navigator.serviceWorker.controller`. Confirm which build answered rather than assuming, by having the worker read its own script — `ctx.serviceWorkers()[0].evaluate(async () => (await (await fetch("/sw.js", { cache: "reload" })).text()).includes("<a string only one version has>"))` — and pick a phrase unique to that version, since one shared by two similar messages in different modules matches both. Worker-side `console.warn` never reaches the page's console capture either; hook it inside the worker with the same `evaluate` and read the record back.
- Responses from the worker's asset endpoints carry `Cache-Control: max-age=31536000, immutable`, so the browser's own HTTP cache answers the second half of a before/after with the first half's bytes. `fetch(url, { cache: "reload" })` escapes it for a raw byte measurement; a rendered `<img>` in the game preview does not, so give each phase its own asset filename — the url is the cache key.
...
- Under Playwright, `Cache.put` fails for the generator's own filtered/thumbnail memo writes on this origin (#477). That costs the memo, not the art: the generator keeps the variant it already built and serves it regardless of the write's outcome. So a second request for the same variant regenerates it instead of hitting the cache; if you are timing asset generation and expecting a cache hit on repeat, you will not see one under Playwright, and that is this, not a regression.
- The command line cannot attach an image to a pull request. Describe what each frame shows under Testing and verification, keep the files in the scratchpad, and attach them through the web form if a person wants them.
- A pid taken from `$!` in Git Bash is the MSYS pid, not the Windows pid the driver probes. Get a child's pid from Node when a state file needs a live one.
- Unlink a scratch junction with `cmd /c rmdir <link>` before removing its directory. `rm -rf <link>` alone removes only the junction, but a trailing slash — `rm -rf <link>/`, which Git Bash tab-completion adds and which is a common way to write the command — follows the junction into the real target and empties it, leaving the junction itself standing.
...
| `up` says `servers pid N are still starting` and then times out             | The launcher the record names lived through the wait and its editor never answered. `down` stops that tree through the record, or removes the record if the launcher has since exited; then `up`. A launcher that exits mid-wait does not reach this: `up` says `have exited; launching` and starts a fresh tree. |
| `status` says `state file unreadable`                                       | The file exists and does not parse. `down` removes it and stops nothing (there is no pid to read); servers it recorded keep running until the machine reclaims them. `up` refuses to launch over it.                                                                                       |
...
| `seed.reason` is set; `error` repeats it; no `gameMounted`                 | The `--project` seed did not leave the whole project in storage; `seed.storage` says what is there and `seed.failed` names the entries. Restarting the servers changes nothing; fix the source (or the storage's refusal) and re-run `--project`. |
| `error` names `local/.seeding` on a run without `--project`                | An earlier seed stopped part-way and storage holds a mix of two projects. Re-run with `--project`, or `seed --clear` to empty it; `ui --probe` still runs on it. |
| `seed.reason` names an entry of the previous project of the other kind     | The previous project has a file where the source has a directory of that name, or the reverse, and the seed removes nothing before it writes. `seed --clear --project <p>` empties the project and seeds it in one run. |
| `seed.reason` says the editor remembers another project                    | The profile once opened a project other than `local` (a Drive project), so the editor opens that one on load. `ui --probe` with a file holding `localStorage.removeItem("project")` forgets it; then re-run. |
| a `ui --sd` step fails with `the game never mounted`                        | The same blank-white failure `verify` reports as `gameMounted: false`, after the same budget and reload retry; `neededReload: true` alone is the retry succeeding. `down`, `up`, retry. |

...
- When timing a scrub, run one throwaway scrub on a line you will not measure and wait a minute before measuring; a click on the line the caret already sits on records nothing, so the throwaway must land on a line other than the one you are about to measure.
...
- A filtered or thumbnailed asset that renders as its unfiltered original is usually not your change: the service worker caches the generated variant and falls back silently when `Cache.put` fails, and under Playwright every `Cache.put` on this origin fails (#477). Before blaming the generator, fetch the variant URL in the page and compare its bytes against the unfiltered one, and check the hover tooltip, which goes through the same path; if both are wrong the same way, it is the worker.
...
- Unlink a scratch junction with `cmd /c rmdir <link>` before removing its directory; `rm -rf` follows a junction into the real `node_modules`.

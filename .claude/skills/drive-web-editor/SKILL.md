...
| `--project <dir-or-zip>` | replace the OPFS project with every file of a directory or an exported zip and reload, as `verify --project` does (section 1); the step carries the `seed` report, fails without reloading on its `reason` (and the steps after it do not run), and needs no `main.sd` in the project when a `--sd` step follows it; a `--sd` before a `--project` is refused before the browser launches, since the seed would remove or replace the script it wrote |

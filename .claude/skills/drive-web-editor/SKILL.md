...
- A `Cache.put` the filtered/thumbnail generator cannot complete costs the memo, not the art (#477): the generator warns and serves the variant it already built, whatever the write did. A second request for the same variant then regenerates it instead of hitting the cache, so a timing run that expects a cache hit on repeat and does not get one is reading this rather than a regression. The warning is worker-side, so it never reaches the page's console capture; hook it inside the worker, as the service-worker gotcha above says, before concluding the write succeeded.
...
- A trailing slash on a scratch junction empties the directory it points at. `rm -rf <link>` removes the junction alone, but `rm -rf <link>/`, which Git Bash tab-completion adds and which is a common way to write the command, follows the junction into the real target and empties it, leaving the junction itself standing. So remove a junction by its bare path, and remove it before the scratch directory around it.

...
- `scrubCheck`: a hint about whether the scrub landed, never the gate; opening the PNG and looking is the primary check, always. Three outcomes, and the middle one is a real answer rather than a soft failure:
...
- When timing a scrub, run one throwaway scrub on a line you will not measure and wait a minute before measuring. The driver refuses a click on the line the caret already sits on and says so (`clicked: false` with a reason naming the unchanged selection) rather than clicking and reporting a success that moved nothing, so the throwaway must land on a line other than the one you are about to measure, or its click is refused instead of warming anything up. The minute is for the preview session itself: a scrub that lands while the first one after a reload is still mid-build races it and mounts a stacked or ahead-of-cursor preview instead of the one that scrub asked for (#456), which would corrupt a timing run far more than it corrupts a screenshot.
...
- Under Playwright, every `Cache.put` on this origin fails when the generator tries to memoise a filtered or thumbnailed variant (#477). That costs the memo, not the art: the generator keeps the variant it already built and serves it regardless of the write's outcome, warning rather than falling back. So a second request for the same variant regenerates it instead of hitting the cache; if you are timing asset generation and expecting a cache hit on repeat, you will not see one under Playwright, and that is this, not a regression.

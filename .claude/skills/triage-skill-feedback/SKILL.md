...
Edit only `groups` in the plan with the editor tool. Keep every open row's key in exactly one nonempty group; counted problems use their `F-...` ID as the key. Read `priorities`, which lists open problems by recorded session count, and weigh severity before choosing work. New counted problems default to `defer`, preserving the open row without filing a Task; frequency is a prioritization signal, not an automatic filing threshold. Group chosen work by one coherent change. The initial groups select `existing` for new feedback on a ticketed problem when that Task is open; verify its scope before applying. Prior references are generated from each row's metadata in published context or the applied summary, including after groups are split; contributor proposals stay separate. Read related tickets before creating a duplicate. Each group takes one of these forms:

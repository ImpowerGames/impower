...
Edit only `groups` in the plan with the editor tool. Keep every open row's key in exactly one nonempty group. Group by one coherent change, splitting the initial per-skill suggestions when they mix unrelated work. The initial groups select `existing` for new feedback on a ticketed section when that Task is open; verify its scope before applying. Prior references are generated from each row's metadata in published context or the applied summary, including after groups are split; contributor proposals stay separate. Read related tickets before creating a duplicate. Each group takes one of these forms:

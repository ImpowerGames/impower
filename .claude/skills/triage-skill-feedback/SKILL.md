...
Missing or changed archive data stops planning. Compare the named index and chunks with saved body artifacts and the original plan's `reportHistory` and `rows`; inspect newer intake and target evidence before selecting a complete version. Restore only verified missing bytes or the matching pointer while preserving unrelated edits, then run report lookup and compare every recovered ID and session set before planning again. If complete history cannot be established, keep intake and ask the maintainer to reconcile it; never reset a count, remove a problem ID or repair an integrity hash to bypass recovery. Storage preflight reserves space for summaries and caps the body and archive comments at a conservative 50,000-character or byte budget; a refusal requires consolidation or a deliberate archive migration before another apply.

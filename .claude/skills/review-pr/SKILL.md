...
Cap the reviewers running on this machine at once to four, counting every session sharing it, not just this round: seven at once, split across two tickets running in parallel, hit the session rate limit and killed a whole round, while four or fewer did not. When another session is spawning reviewers too, or a High-impact round's own tier would put this session's share of that shared cap over four, split the round's lenses into two batches and spawn them one after the other rather than dropping a lens.

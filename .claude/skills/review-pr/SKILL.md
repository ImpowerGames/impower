...
A definition has to be picked up by the harness before it can be spawned, and that can lag the file landing on disk; a newly added one showed up part-way through a run rather than at the moment it was written. Go by your agent-type list, not by the file. A reviewer this session cannot spawn — because the repository defines no agent for it, or this session's agent-type list does not carry it — is a refusal to review, never a substitution: adding a definition does not make it usable in the same session, and a pin the session cannot confirm is being served resolves silently to the writer's own model. While a name is absent from it, fall back to `subagent_type: "general-purpose"` and an explicit `model:`:

...
Two reviewer definitions under `.claude/agents/` belong to this skill: `reviewer-opus-4-6.md` and `reviewer-opus-5.md`, each pinning a full model id in its frontmatter. They live in `.claude/agents/` because that is the only directory the harness reads project subagents from. `reviewer-model-values.test.sh` beside this file pins the table in section 2 and the prompt in section 3 against routing a writer to its own model; run it after any edit here, to a definition, or to the script itself. Its controls run the same check function in the same process against forty-nine fixtures, six of them correct and the rest deliberately broken, and two re-execute the file on a fixture to pin its exit status and closing lines, so the whole run, 60 assertions, takes seconds (5s on this machine) and needs bash 4.4 or newer; the result is the closing line, `All reviewer-model assertions passed, 52 controls included.` Given a skill file and an agents directory as arguments it checks those alone and its closing line says the controls did not run.

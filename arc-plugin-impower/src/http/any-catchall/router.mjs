...
import enhance from "../../transforms/ssr/enhance.mjs";
import styleTransform from "../../transforms/styleTransform.mjs";
...
export default async function api(options, req) {
  ...
  if (altPath) {
    let apiPathPart =
      apiPath && apiPath.replace(path.join(basePath, "api"), "");
    let pagePathPart =
      pagePath && pagePath.replace(path.join(basePath, "pages"), "");

    let altApiPath = getModule(altPath, "api", req.rawPath);
    let altPagePath = getModule(altPath, "pages", req.rawPath);
    let altApiPathPart =
      altApiPath && altApiPath.replace(path.join(altPath, "api"), "");
    let altPagePathPart =
      altPagePath && altPagePath.replace(path.join(altPath, "pages"), "");
    if (!apiPath && altApiPath) {
      ...
      apiPath &&
      altApiPath &&
      compareRoute(apiPathPart, altApiPathPart) === -1
    ) {
      apiPath = altApiPath;
      apiBaseUsed = altPath;
    }
    if (!pagePath && altPagePath) {
      ...
      pagePath &&
      altPagePath &&
      compareRoute(pagePathPart, altPagePathPart) === -1
    ) {
      pagePath = altPagePath;
      pageBaseUsed = altPath;
    }
  ...
  if (apiPath) {
    ...
    try {
      ...
      throw new Error(`Issue when trying to import API: ${apiPath}`, {
        cause: error,
      });
    }
    ...
    if (isAsyncMiddleware) method = arc.http.async.apply(null, method);
    if (method) {
      ...
      if (isAsyncMiddleware) {
        ...
        if (state.isBase64Encoded) {
          let encoding =
            state?.headers?.["content-encoding"] ||
            state?.headers?.["Content-Encoding"];
  ...
  const store = state.json ? state.json : {};
  ...
  try {
    ...
    if (altPath && !fiveHundred)
      fiveHundred = getModule(altPath, "pages", "/500");
    let body = "";
    if (fiveHundred && fiveHundred.includes(".html")) {
      let raw = read(fiveHundred).toString();
      body = html`${head({ req, status, error, store })}${raw}`;
    } else {
      body = html`${head({ req, status, error, store })}<page-500
          error="${error}"
        ></page-500>`;
    }
    return { status, html: body };

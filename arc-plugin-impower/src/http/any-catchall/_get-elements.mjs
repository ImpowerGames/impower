...
export default async function getElements(basePath) {
  ...
  if (exists(pathToHead) === false) {
    ...
    try {
      ...
      throw new Error("Issue when trying to import head file.", {
        cause: error,
      });
    }
  }
  ...
  if (exists(pathToModule)) {
    ...
    try {
      ...
      throw new Error("Issue when trying to import elements manifest.", {
        cause: error,
      });
    }
  }
  ...
  if (exists(pathToPages)) {
    ...
    for (let p of pages) {
      ...
      try {
        ...
        throw new Error(`Issue when trying to import page: ${p}`, {
          cause: error,
        });
      }
    }
  ...
  if (exists(pathToElements)) {
    ...
    let files = getFiles(basePath, "elements").filter((f) =>
      f.endsWith(".mjs")
    );
    for (let e of files) {
      // turn foo/bar.mjs into foo-bar to make sure we have a legit tag name
      const fileURL = pathToFileURL(e);
      let tag = fileURL.pathname
        .replace(elementsURL.pathname, "")
        .slice(1)
        .replace(/.mjs$/, "")
        .replace(/\//g, "-");
      if (/^[a-z][a-z0-9-]*$/.test(tag) === false) {
        throw Error(
          `Illegal element name "${tag}" must be lowercase alphanumeric dash`
        );
      }
  ...
  if (!els["page-404"]) els["page-404"] = _404;

  if (!els["page-500"]) els["page-500"] = _500;

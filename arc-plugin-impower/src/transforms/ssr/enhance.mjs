import { fragment, parse, serialize, serializeOuter } from "@begin/parse5";
import { customAlphabet } from "nanoid";
import isCustomElement from "./is-custom-element.mjs";
import { decode, encode } from "./transcode.mjs";
...
export default function Enhancer(options = {}) {
  ...
  return function html(strings, ...values) {
    ...
    const { collectedStyles, collectedScripts } = processCustomElements({
      node: body,
    });
    ...
    return (
      bodyContent ? serializeOuter(body.childNodes[0]) : serialize(doc)
    ).replace(/__b_\d+/g, "");
  };
}

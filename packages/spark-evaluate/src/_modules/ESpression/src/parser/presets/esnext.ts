...
import { TryBranchRule } from "../rules/tryBranch";
...
export function esNextRules(
  ...
  const rule = rules[NOCOMMA_EXPR];
  if (rule) {
    rule[1] = new BinaryOperatorRule(
      opConf(
        [
          "=",
          "+=",
    ...
    new BinaryOperatorRule({
      "?.": {
        ...
        extra: (n) => ({
          ...
          shortCircuited: !!(
            n.object &&
            (n.object.shortCircuited || n.object.optional)
          ),
        }),
      },
      ...
      "?.(": {
        ...
        extra: (n) => ({
          ...
          shortCircuited: !!(
            n.callee &&
            (n.callee.shortCircuited || n.callee.optional)
          ),
        }),
      },
  ...
  CALL_TYPE.trailing = restoreComma;
  return rules;
}

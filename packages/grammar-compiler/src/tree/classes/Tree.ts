...
export class NodeProp<T> {
  ...
  /// it does.
  add(
    match: { [selector: string]: T } | ((type: NodeType) => T | undefined)
  ): NodePropSource {
    if (this.perNode)
...
export class NodeType {
  ...
  static define(spec: {
    ...
    let flags =
      ...
      (spec.skipped ? NodeFlag.Skipped : 0) |
      (spec.error ? NodeFlag.Error : 0) |
      (spec.name == null ? NodeFlag.Anonymous : 0);
  ...
  get isTop() {
    return this.flags != null && (this.flags & NodeFlag.Top) > 0;
  }

  /// True when this node is produced by a skip rule.
  get isSkipped() {
    return this.flags != null && (this.flags & NodeFlag.Skipped) > 0;
  }

  /// Indicates whether this is an error node.
  get isError() {
    return this.flags != null && (this.flags & NodeFlag.Error) > 0;
  }

  /// When true, this node type doesn't correspond to a user-declared
  /// named node, for example because it is used to cache repetition.
  get isAnonymous() {
    return this.flags != null && (this.flags & NodeFlag.Anonymous) > 0;
  }
...
export class NodeSet {
  ...
  extend(...props: NodePropSource[]): NodeSet {
    ...
    for (let type of this.types) {
      ...
      newTypes.push(
        newProps ? new NodeType(type.name, newProps, type.id, type.flags) : type
      );
    }
...
function buildTree(data: BuildData) {
  ...
  function takeNode(
    parentStart: number,
    minPos: number,
    children: (Tree | TreeBuffer | ITreeBuffer)[],
    positions: number[],
    inRepeat: number,
    depth: number
  ) {
  ...
  function makeTree(
    ...
    if (contextHash) {
      let pair: [number | NodeProp<any>, any] = [

 ...
 * since it runs the beat once the checkpoint is loaded and the preview then
 * displays that run; the hint is issued before the route is even planned,
 * when nothing can run, so it guesses. A hint's loads hold express slots the gate then finds taken, so a

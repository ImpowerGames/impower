 ...
 * since the preview runs the beat and waits for the pictures it flushed
 * before it writes them; the hint is issued before the route is even
 * planned, when nothing can run, so it guesses. A hint's loads hold express slots the gate then finds taken, so a

...
/** A program's summary, as the page holds it; the screen is the worker's
 *  game. */
...
function harness() {
  ...
  // What PLAY names: the real program, by its identity, for the worker to
  // build PLAY's game from.
  const playable = programIdentity(REAL);
  ...
  const shown: { name: string; speculative: boolean }[] = [];
  let draws = 0;
  ...
  controller.updatePreview = async (
    ...
    const draw = ++draws;
    controller._workerGame.program = p;
    if (gate) await gate;
    if (draw !== draws || options?.current?.() === false) return false;
    shown.push({ name: p.name, speculative: !!options?.speculative });
    return true;
  };
  let request = 0;
...
describe("the suggestion preview", () => {
  describe("while a suggestion list is open", () => {
    test("one suggestion is compiled at a time, and only the newest waiting one follows", async () => {
      ...
      await answer("angry");
      expect(shown).toEqual([{ name: "angry", speculative: true }]);
    ...
    test("a project file change while a suggestion is shown compiles it again", async () => {
      ...
      await controller.loadProgram(program("real, new files", 2));
      await settle();
    ...
    test("a selection in another document ends the suggestion preview", async () => {
      ...
      await controller.loadProgram(program("real, other", 2));
    ...
    test("real programs compiled meanwhile are kept but not shown", async () => {
      ...
      await controller.loadProgram(newer);

      expect(shown.map((s) => s.name)).toEqual(["happy"]);
      expect(controller._program).toBe(newer);

      await close();
      expect(shown.at(-1)).toEqual({ name: "real, typed", speculative: false });
    });
  ...
  describe("when the list closes", () => {
    test("the real document is shown at once, and a compile still in flight never lands", async () => {
      ...
      expect(shown.at(-1)).toEqual({ name: "real", speculative: false });
      await answer("sad");
    ...
    test("real programs held back while it was open are shown, even if no suggestion was", async () => {
      ...
      focus(null);
      await settle();
      await controller.loadProgram(program("real, typed", 2));
      expect(shown).toEqual([]);

      await close();
      expect(shown).toEqual([{ name: "real, typed", speculative: false }]);
      expect(controller.getGameState().completion).toBeNull();
    });
    ...
    test("an accepted suggestion on screen stays until its own program arrives", async () => {
      ...
      await controller.loadProgram(program("before acceptance", 1));
      expect(shown.map((s) => s.name)).toEqual(["happy"]);
      await controller.loadProgram(program("accepted", 2));
      expect(shown.at(-1)).toEqual({ name: "accepted", speculative: false });
    });
    ...
    test("a real document that stopped compiling while it was open is marked, even if no suggestion was shown", async () => {
      ...
      focus(null);
      await settle();
      await controller.loadProgram(program("does not compile", 2, null));
      await close();
    ...
    test("a real document that does not compile keeps the last frame, marked", async () => {
      ...
      await controller.loadProgram(program("fixed", 3));
  ...
  describe("a preview that is not open and stopped", () => {
    test("takes no part while the game is running", async () => {
      ...
      controller._workerPlay = { program: controller.screenProgram, state: "running", stepped: 0 };
      focus("happy");
      await settle();
      expect(compiles).toEqual([]);
    });
  ...
  describe("what the game reports while it shows a suggestion", () => {
    test("is not passed on as the author's", async () => {
      const { controller, focus, answer } = harness();

...
// for the game.
//
// With the worker displaying the preview (#680), the same steps run on the
// worker's game (`displayPreviewFrom`) and the controller waits for the
// worker's answer, so every ordering below is checked in both positions: the
// page's `updatePreview` over a game on the page, and `displayPreviewFrom`
// over the worker's.
...
/**
 * The display of `program` at `file`/`line` over `game` in one switch
 * position: the page's `updatePreview`, or the worker's `displayPreviewFrom`
 * with the game connecting through the same stand-in application, so a test
 * that holds the connect holds it in either. Answers whether the frame was
 * completed.
 */
function displayIn(worker: boolean, game: any, app: any) {
  ...
  return {
    ...
    display(
      ...
      if (!worker) {
        return controller.updatePreview(program, file, line, "SAVE", undefined, options);
      }
...
for (const worker of [false, true]) {
  describe(`preview session ordering (${worker ? "the worker's game" : "the page's game"})`, () => {
    test("the game is told it is previewing before the load and the connect", async () => {
      const calls: string[] = [];
      await displayIn(worker, recordingGame(calls), stubApp(calls)).display(PROGRAM, PROGRAM.uri, 4);
    ...
    test("a repeated path still waits for the engine's pending image gate", async () => {
      ...
      let settled = false;
      const updating = displayIn(worker, game, stubApp([]))
    ...
    test("marks the path the remembered point resolves to now, not the one it resolved to before", async () => {
      ...
      await displayIn(worker, game, stubApp(calls)).display(program, "file://proj/other.sd", 1);
    ...
    test("marks nothing for a remembered point that no longer resolves, and still previews", async () => {
      ...
      await displayIn(worker, game, stubApp(calls)).display(program, "file://proj/third.sd", 1);
    ...
    test("marks the game's own path for the remembered point while the program stands", async () => {
      ...
      await displayIn(worker, game, stubApp(calls)).display(PROGRAM, PROGRAM.uri, 9);
    ...
    test("the game leaves the editors' part out of its report while it shows a suggestion, and puts it back for the document", async () => {
      ...
      const displaying = displayIn(worker, game, app);
    ...
    test("a project that resolves no path still reconnects on every selection", async () => {
      ...
      game.program = { uri: PROGRAM.uri, version: PROGRAM.version };
      await displayIn(worker, game, stubApp(calls)).display(PROGRAM, PROGRAM.uri, 4);
    ...
    test("the elements the last preview left are not swept until the preview has settled", async () => {
      ...
      const updating = displayIn(worker, game, stubApp(calls)).display(PROGRAM, PROGRAM.uri, 4);
      for (let i = 0; i < 10; i++) {
    ...
    test("the preview is not written until the connect has settled", async () => {
      ...
      const updating = displayIn(worker, recordingGame(calls), app).display(PROGRAM, PROGRAM.uri, 4);
      for (let i = 0; i < 10; i++) {
        await Promise.resolve();
      }
      expect(calls).toContain("connectGame");
    ...
    test("a display another overtakes while it waits does not sweep the newer display's pass", async () => {
      ...
      const displaying = displayIn(worker, game, stubApp(calls));
    ...
    test("a display overtaken while it waits for the connect neither previews nor sweeps", async () => {
      ...
      const displaying = displayIn(worker, recordingGame(calls), app);
      const first = displaying.display(PROGRAM, PROGRAM.uri, 4);
      for (let i = 0; i < 10; i++) {
        await Promise.resolve();
      }
  ...
  describe(`the controller's preview updates (${worker ? "the worker displaying" : "the page displaying"})`, () => {
    afterEach(() => setWorkspace(undefined as any));

    /**
     * A controller in one switch position. With the worker displaying, the
     * page holds the program's summary and each display is a request the
     * test answers; `finishDisplay` settles the oldest one still waiting.
     */
    function controllerIn(calls: string[], holdDisplays = false) {
      if (!worker) {
        ...
        return { controller, program: PROGRAM, finishDisplay: () => finishes.shift()?.() };
      }
      const waiting: ((result: { displayed: boolean }) => void)[] = [];
      setWorkspace({
        workerDisplaysPreview: true,
        gameLink: {
          detach() {},
          request: () => {
            calls.push("preview");
      ...
      const controller: any = controllerWith(undefined, stubApp(calls));
...
describe("the page's own preview game", () => {
  test("a game the play path is still building is left alone", async () => {
    ...
    // it previewing would cost that run its renderer. The worker's game never
    // plays, so this is the page's alone.
    const calls: string[] = [];
    const controller = controllerWith(recordingGame(calls, "initial"), stubApp(calls));

...
3. `morphSubpaths(from, to, { method })` interpolates one pair with `nodes`, `taper` or `trace`. Compound drawings pair subpaths in document order, only with equal counts. Every subpath must be closed, by `Z` or by ending within `seamTolerance` of its start; an open one is `not-closed`. A subpath identical in both drawings stays still. There is no automatic method selection: a method that cannot handle the geometry returns `{ ok: false, failure }` with a code (`node-count`, `tips-not-found`, `self-intersection`, `thickness`, `not-closed`, `subpath-count`, `empty-geometry`) so the caller can apply the author's chosen fallback. The low-level builders `nodesTrack`, `taperTrack` and `traceTrack` return the same structured failures, including on empty input.

...
- No frame self-intersects at the eight bake samples or at 21 evenly spaced runtime progress values; the methods check nine progress values by default because a crossing that opens and closes between two coarser samples otherwise slips through. The check flattens the whole loop into one polyline, so two adjacent curves that cross away from their shared anchor, a loop of only one or two curves, and a knot inside a single curve are all caught. Parallel-segment and straight-segment tests are relative to the segments' own lengths, so they hold at any scale.

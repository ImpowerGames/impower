...
const shapes = (weights: number[], paths: string[]) =>
  palettes.flatMap((p) =>
    weights.map((w) => [
      ...(paths[0] ? [{ path: paths[0], fillColor: p[0] }] : []),
      ...(paths[1]
        ? [{ path: paths[1], strokeColor: p[1], strokeWeight: w }]
        : []),
      ...(paths[2]
        ? [{ path: paths[2], strokeColor: p[2], strokeWeight: w }]
        : []),
      ...(paths[3]
        ? [{ path: paths[3], strokeColor: p[3], strokeWeight: w }]
        : []),
      ...(paths[4]
        ? [{ path: paths[4], strokeColor: p[4], strokeWeight: w }]
        : []),
    ])
  );

export const GRAPHIC_RANDOMIZATIONS: Record<
  string,

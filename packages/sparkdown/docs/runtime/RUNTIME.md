...
Two flags change the closing newline. A table with `open` writes none: the next display call joins its line, and the step runs on until a call closes it. A table with `caption` (a `choose` block's last caption line) leaves the newline pending (`StoryState.lineEndPending`): if the step reaches its choices, or the story's end, without showing anything else, it completes with the caption and the choices together. The first output that shows something (text that is not only whitespace, a display table or a tag) marks the output stream where it starts (`outputCut`). The continue ends after that step: its output ends at the mark with the pending newline, and what the step showed past the mark moves to the flow's `carriedOutput`, which is saved with the flow and starts the next continue's output, so nothing is run twice. A step that leaves the story unable to continue keeps its output whole, with the newline written at the mark, since no continue follows to carry it to. A call that runs against an output stream of its own (`EvaluateFunction`, `CallLuauFunction`, `pcall`) suspends the pending newline, the cut and the carried output while it runs, since what it shows never reaches the story's steps; a host calling `EvaluateFunction` between continues leaves the carried output for the story's next continue.

...
Ink's `<>` glue becomes a `..` that ends a display line: the next display line joins onto it, in the same beat. The join keeps the spaces written before the `..` and drops the spaces after it, so `A ..` then `B` shows "A B", `A   ..` keeps all three spaces, and `A..` joins with none. Anything but a dot may touch the `..`, so `Wait...` stays an ellipsis. A `..` in the middle of a line (`a..b`, `a .. b`) is text, except right after a break (below), and tags or a `//` comment may follow the `..` at the end.
...
A `..` after a `>` break waits for the click and then carries on in the same box, as Ren'Py's `extend` does. It may end the line (`Hold > ..` then `on tight.`, or `Abso >..` then `lutely.`) or stand in the middle of one (`Hold > .. on tight.`, `Hold > ..on tight.`, `Abso >..lutely.`), with a space before the `>` as every break needs; the spaces after a mid-line `..` are dropped. After the click the box keeps the text it shows, which is not typed again, and only the continuation is typed after it, with its own typing sound: "Hold on tight." and "Absolutely.". The space before the `>` belongs to the break, so it is dropped, while the spaces between the `>` and the `..` are kept. A `>` that touches the word before it (`Hold>..`) is text, not a break, and the compiler warns about it. Chains work: in `One > .. two > .. three.` each of the two clicks adds to the box.

What the continuation brings replaces what the box shows. A continuation with a cue (`???: Hello... > ..` then `ALICE: It's me.`) shows ALICE's name over "Hello... It's me." and is typed with ALICE's settings, and a parenthetical on its cue or on a line of its own before its words replaces the one shown; without them the box keeps its own. The kind of box stays the first part's: an action box extended by `BOB: Who's there?` shows the words and no name. The box keeps its portrait unless the continuation shows another, and a sound or voice the first part started keeps playing unless the continuation starts another on its channel. Blank lines, comments, asset lines and logic between the break and the continuation belong to the continuation and run at the click. The continuation is the next line the run shows, as for any join, so it may be reached through an `if` branch, a divert or the top of another scene. When the branch taken of an `if` or alternator block after the break shows nothing, the line right after the block starts a new box; a `> ..` inside the branch taken leaves its own box, which that line carries on in. A logic line between the block and the next line hides the block, so that line carries on in the box. A continuation of only pictures, sound or a `load` line runs them at the click, keeps the box on the page and adds no text, and the next text carries on in the box. Choices after the continuation attach to the box as to any box, and choices end it.

The break's table carries `pause` and `extend` and writes its newline, so each part is a step of its own: the lines of the first part preview it alone, the lines after its break preview the whole box, and a checkpoint or PLAY from a continuation shows the whole box with the earlier text at once and the continuation typed. The interpreter keeps the box it showed in its saved state, and the next step that shows text, or that a break ends, carries on in it. A `..` before the break (`A .. >` then `B`) is an ordinary join instead: the player sees "A B" in one box, then the click, then a new box, and the joined line keeps the first line's speaker.

...
A `choose` block holds the flow: once its choices are offered, nothing after the block runs until one is taken. Every choice that does not divert away continues at the block's `end`, runs the `then` body if there is one, and goes on to whatever follows the block, at any nesting: the rest of an `if` arm the block sits in, then what follows that `if`, and so on out. The `then` body is optional. A block that offers no choice, because every choice in it is gated off, holds nothing: the flow runs through its `then` body and on past it. A `* ->` fallback choice is taken when no other choice is available. A thread started in a block's preamble (`<- name`) adds its choices to the block's, so the block holds for them; taking one continues in that thread. A `choose` block written in another block's preamble, before its first choice or inside an `if` there, offers its choices together with that block's; its choices continue at that block's `end`, and what follows it in the preamble runs before the choices are offered.

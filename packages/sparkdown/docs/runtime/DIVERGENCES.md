...
The display lines written in a `choose` block before its first choice are its caption, the line the choices answer. The last of them shows together with the choices: its call is `open`, so the step runs on through any logic between it and the first choice and completes with the caption and the choices at once. Earlier caption lines are steps of their own, and so is a caption line followed, before the first choice, by a conditional or an alternator that can show something or divert, or by a divert, a tunnel return, text, a tag or glue. A conditional that only runs logic is logic. Write the line the choices answer inside the block, as in the example above, so it stays on screen while the player picks.

...
| `Whitespace`         | `[\t ]+`             | mid-line whitespace that should be preserved as-is (dialogue body text, the spaces before a line-ending `..`, break pacing). Also the natural pick for leading-WS captures: the formatter still normalizes those at line start. |
...
**After a leading `..`.** A `LeadingGlue` node is the `..` that begins `InlineAction`, the text after the colon of any other inline display line, or a block body line. At the start of a line's text it marks the line as joining the line shown before it when that one ends with `..`, which the engine checks as the story runs (`docs/runtime/RUNTIME.md` §3.1), and the line shows its text without the mark or the spaces after it. The grammar also reads the rest of a line after a top-level interpolation as an `InlineAction`, so in `{x} ..   more` the node is a `..` in the middle of the line, which the player shows as text together with the spaces after it. In both cases the whitespace after the mark is the author's, so the formatter leaves a `RequiredWhitespace` or `OptionalWhitespace` run that begins where a `LeadingGlue` ends as written: `.. B`, `..B` and `..   B` keep their spacing, and so does `{x}..   more`.
...
The `Sparkdown`-prefixed variant is generally a _superset_ of the un-prefixed one — it accepts everything the Luau version accepts plus display-context constructs. The return pair is the exception: `LuauSparkdownReturnStatement` accepts less, because a Luau value on the line after a bare `return` would be prose in a narrative body. Narrative code also reaches `LuauReturnStatement` itself, through `LuauExplicitStatement` and `LuauExpression`, which both bodies share; that is why it reads the next line only for a `return` that starts its line, whatever the context. An `&` statement can place `repeat` or a ternary `else` directly before `return`, so a block keyword before it is not enough. **If you add a new block-shaped construct that should work in both contexts, you almost always need to write both variants** (and remember §3.2 — each variant's `patterns:` must be exhaustive for its context).

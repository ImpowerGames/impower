...
- `buildDisplayCall(target, character, body, range, ctx, tags, { pause, extend, nested, fresh, inherit, group, open, continues })` makes `display({ target?, character?, text, pause?, extend?, nested?, fresh?, inherit?, group?, open?, continues?, tags? })`. The routing is resolved at compile time: a dialogue line names `target = "dialogue"` and its cue as `character` (the name, then the parenthetical written after it, `readCue`), a write line names its layer, and every other line type names itself. A table with no `target` renders on the default target. `text` is a `StringExpression` over the body's ParsedObjects, so interpolations are evaluated when the call runs. The call is stamped with `range`, which gives its beat a path location.
...
- A spaced `>` break anywhere in a display statement splits it into one call per beat (`splitBodyRangeAtBreaks`), and each beat a break ends carries `pause`. A break with no text before it adds a text-free beat in an inline line, stamped where the break stands, and only splits a block. The tags written after a break that ends its line stay with the beat before it. A glued continuation's calls carry `group`, which names the continuation by its file and the offset it starts at, since offsets start again in every script. Its beats after one of its breaks also carry `inherit`, because their routing is that of the beat the continuation joins, which only the run knows; they name the routing the source reads for the line before the continuation (`lexicalRouting`, which follows chains of continuations) as the answer for a run that never joined them. A `..` that ends a statement marks its last call `open`: `display` writes no newline after it, so the step runs on, and the next display line is a continuation whose table names no target, which joins the beat of the line it continues. The first statement of an `if`, `elseif` or `else` branch or of an alternator arm runs right after whatever ran before the block, so it continues the line before the block when that line ends with `..` (`precedingConstruct`). A `..` after a break (with a space before the `>`), ending the line (`A >..`, `A > ..`) or followed by words (`A > .. B`, `A >..B`), marks the beat its break ends `extend` rather than `open`: its call writes its newline, so the part after the click is a step of its own, which the interpreter shows in the same box. The spaces between the `>` and the `..` end that beat's text, as the spaces the join keeps. The line after a `> ..` names its own routing (`isNodePrecededByExtend`), since a continuation that names a speaker names the box's new one. A `..` before a break (`A .. >`) makes the call `open` as any line-ending `..` does. The first call of a display line whose preceding construct is an `if` or alternator block carries `fresh` (`followsBranchingBlock`), and the `extend` call of a line inside such a block carries `nested` (`insideBranchingBlock`): the interpreter ends a waiting box at `fresh` unless it is `nested`. Only the kind of the construct right before the line is read, since the incremental compiler lowers a line again when that construct changes but not when a line deep inside it does. A `..` that ends a body line inside a block is resolved when the body is lowered (`joinMidBodyGlue`). A `load` line ending with `..` is a reported error, and no join lowers to a `Glue` object. Whether a beat is a `load` directive is decided per beat, as lowering decides it: `isLoadRange` asks `splitBodyRangeAtBreaks` for the beat that holds the `..`, over the body the statement's lowerer reads, so it agrees with lowering about where a beat starts (after a break's trailing tags, past a leading `..`). A `..` that ends a `load` beat never joins, and a beat of text after a break in a `load` statement joins as any other. A `..` that begins a line joins nothing: it states that the line continues the one before, and the line lowers as it would without the mark and the spaces after it. `checkLeadingGlue` reports from one verdict, `leadingGlueCheck`, which records whether words follow the mark and what the line before is: the body line above it in a block, past blank and comment lines, and otherwise the construct `precedingConstruct` returns. A mark with no words after it (only tags or a comment) is an error whose fix depends on the line before: under a line that ends with `..` it asks for the missing words, and otherwise for a `..` on the line before. When the line before is a display line (a display statement or a line of interpolations) that does not end with `..`, the mark is an error, and when it is a `load` beat the error says that line cannot join, whether or not words follow the mark. When it is nothing, or a construct that is not a display line (a logic statement, a divert, the end of a block), only the run knows the line before, so the statement's first call carries `continues` (`runCheckedLeadingGlue`) and `display` checks the line is open as it runs (`docs/runtime/RUNTIME.md` §3.1). The compiler does not look past a statement that is not a display line. The grammar reads what follows a top-level interpolation on its line as an action of its own, so the marks that end such a line (`{x} ..`, `{x} >..`, `{x} > >`) make that line's beats (`lineEndMarks`): the interpolation's call is the first, each further `>` adds an empty beat that waits, and a `..` opens the last, or marks it `extend` when it follows a `>`. A `..` with anything else after it there is text. A `>` that touches the word before a line-ending `..` is text, and the compiler warns about it.

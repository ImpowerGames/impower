...
- The display lines of a `choose` block before its first choice are its caption. The last caption call is `open` (`openCaption` in `lowerSparkdownChooseBlock.ts`) when only logic stands between it and the first choice, so the step that shows it runs on to the choices; a conditional, an alternator, a divert or text in between leaves it closed.

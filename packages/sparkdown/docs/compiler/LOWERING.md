...
- The display lines of a `choose` block before its first choice are its caption, and the last caption call is `open` (`openLastDisplayCall`), so the step that shows it runs on to the choices.

...
- The display lines of a `choose` block before its first choice are its caption. The last caption call is `open` (`openCaption` in `lowerSparkdownChooseBlock.ts`) when only logic stands between it and the first choice, so the step that shows it runs on to the choices. Anything in between that can show something or divert leaves it closed: text, a tag, glue, a choice, a divert or a tunnel return anywhere inside it, or a call to a stdlib function that is not pure (`display` and `print` show, and `pcall` runs code) or to a function the story defines. Assignments, pure stdlib calls, calls to externals and conditionals holding only those do not. The program's externals may be declared in any included script, which lowering cannot see, so the names of the other calls in between travel on the `open` entry as an `ExternalsExpression` (`compiler/lower/utils/ExternalsExpression.ts`), which evaluates to true only when every one resolves to an external; it is settled in `ResolveReferences`, after the story has collected every external.

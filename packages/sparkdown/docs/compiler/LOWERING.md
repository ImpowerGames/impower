...
- The display lines of a `choose` block before its first choice are its caption. The last caption call is `open` (`openCaption` in `lowerSparkdownChooseBlock.ts`) when only logic stands between it and the first choice, so the step that shows it runs on to the choices. A conditional or an alternator that can show something or divert, a divert, a tunnel return, a nested weave, text, a tag or glue in between leaves it closed; a conditional that only runs logic does not.

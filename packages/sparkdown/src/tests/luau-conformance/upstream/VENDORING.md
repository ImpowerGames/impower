# Vendored Luau upstream tests

This directory holds two things from [luau-lang/luau](https://github.com/luau-lang/luau), each pinned to its own upstream commit: a verbatim copy of `tests/conformance/`, preserved here so sparkdown can run Luau's own test suite against our Luau-superset implementation, and a generated list of the cases in Luau's type-checker tests (see "Type-checker test cases" below).
...
- `typecheck-cases.json` — the type-checker test cases, generated by `packages/sparkdown/scripts/generateTypecheckCases.ts`.
...
- Upstream commit `conformance/` was copied from: `c8cf2864adec33eb4eb5b4cc7e0708aa74893ba0`
  (2026-05-15, "Sync to upstream/release/721 (#2394)").

Update this entry whenever you re-vendor `conformance/`. `typecheck-cases.json` has its own pin, below.

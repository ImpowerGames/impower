...
// NAME entering or leaving the program, a list definition (still inlined by
// value), a change to the `include`/`run`/`EXTERNAL` structure, or a callee's
// parameter list — and each of those is detected wherever in the document it
// is written.
//
// So these tests pair each case with the position it sits at, and assert the
// reuse DECISION rather than only that the output matches a cold compile: a
// flow served from a stale cache and a flow rebuilt from scratch produce the
// same bytes whenever the cache was valid, so output equality alone cannot
// tell a working cache from a disabled one.
...
function fixture(): string {
  ...
  for (let s = 0; s < SCENES; s++) {
    ...
    lines.push(`  Line ${s} of the fixture.`);

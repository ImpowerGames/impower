...
// The incremental compile reuses a scene's constructed flow only when every
// chunk of its run reappears unchanged and nothing new attaches to it, and a
// function declaration must not hide a new line that attaches past it. Each
// edit below is applied to one persistent compiler and compared with a cold
...
interface Edit {
  ...
  replace: string;
  after?: string;
  /** A scene this edit's compile must reuse. */
  reuses?: string;
}
  ...
  try {
    ...
    for (const edit of edits) {
      ...
      if (edit.reuses) {
        ...
        expect(reused, `reused after edit "${edit.name}"`).toContain(
          edit.reuses,
        );
      }
      expect(stable(incrProg), `after edit "${edit.name}"`).toBe(
...
describe("incremental compile with story lines after a function declaration", () => {
  it("a line added after a function that ends a scene joins the scene", () => {
    ...
    expectIncrementalMatchesCold(base, [
      editLast("L\n", "L1\n", "one"),
      editLast("L1\n", "L2\n", "one"),
      { name: "add a line after the function", find: "end\n\n", replace: "end\nA2\n\n", after: "return" },
      editLast("L2\n", "L3\n", "one"),
      { name: "edit the line after the function", find: "A2\n", replace: "A3\n" },
      editLast("L3\n", "L4\n", "one"),
  ...
  it("a function added inside a scene leaves the lines after it in the scene", () => {
    ...
    expectIncrementalMatchesCold(base, [
      editLast("L\n", "L1\n", "one"),
      { name: "add a function between two lines", find: "A\nC\n", replace: `A\n${LESS}C\n` },
      editLast("L1\n", "L2\n", "one"),
      { name: "edit the line after the function", find: "C\n", replace: "Cee\n", after: "return" },
      editLast("L2\n", "L3\n", "one"),
  ...
  it("lines after a function declared first stay in the root flow across edits", () => {
    ...
    expectIncrementalMatchesCold(base, [
      editLast("L\n", "L1\n", "filler_1"),
      { name: "edit a root line", find: "A\n", replace: "Aye\n", after: "return" },
      { name: "edit the function body", find: "a < b", replace: "a <= b" },
      editLast("L1\n", "L2\n", "filler_1"),
      { name: "add a second function", find: "Aye\n", replace: `function more(a, b)\n  return a > b\nend\nAye\n` },
      { name: "add a root line after the second function", find: "Aye\n", replace: "Aye\nZ\n" },
      editLast("L2\n", "L3\n", "filler_1"),
    ]);
  });
});

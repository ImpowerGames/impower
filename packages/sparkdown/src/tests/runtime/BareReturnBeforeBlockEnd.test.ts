...
//
// The same end pattern of `LuauReturnStatement` is also pinned by
// `luau-conformance/ReturnValueNextLine.test.ts` (a value or block end on the
// line after `return`) and `compiler/returnBeforeProse.test.ts` (a narrative
// `return` ends at its line).
...
describe("a bare return before a block-closing keyword on its line", () => {
  test.each([
    ...
    ["a for loop", "for i = 1, 1 do return end"],
  ...
  test("an if block's return leaves the rest of the function to run when skipped", () => {
    const r = runConformanceSource(`local function f(c) if c then return end return "after" end
assert(f(true) == nil)
assert(f(false) == "after")`);
...
describe("a return that already parsed before a block-closing keyword", () => {
  test.each([

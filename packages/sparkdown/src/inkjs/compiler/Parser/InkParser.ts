...
export class InkParser extends StringParser {
  ...
  public readonly PropertyInitialization = (): Expression | null => {
    ...
    if (expr) {
      if (expr?.debugMetadata) {
        unquotedStringExpr.debugMetadata = new DebugMetadata(
          expr?.debugMetadata,
        );
      }
    }
  ...
  public readonly QuotedIdentifier = (): Identifier | null => {
    ...
    const id = this._chars
      .slice(stateAtStart.characterIndex + 1, this.index)
      .join("");
  ...
  public readonly ParseWhitespace = (): string | null => {
    const result = this.ParseCharactersFromCharSet(this._inlineWhitespaceChars);

    return result;
  };

  public readonly Whitespace = (): typeof ParseSuccess | null => {
    const doneParsed = this.ParseWhitespace();

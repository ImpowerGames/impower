...
function closeLintPanelAndRestoreFocus(view: EditorView) {
  closeLintPanel(view);
  view.focus();
  view.dispatch({ selection: view.state.selection.main });
}

function closeReferencePanelAndRestoreFocus(view: EditorView) {
...
export class StatusPanel implements Panel {
  ...
  toggleBottomPanel(state: EditorState) {
    if (isLintPanelOpen(state) || isReferencePanelOpen(state)) {
      closeLintPanelAndRestoreFocus(this.view);
      closeReferencePanelAndRestoreFocus(this.view);
    } else {

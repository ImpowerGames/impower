...
import type { LanguageData, TextmateData } from "../grammar/types/definition";
import {
...
import type { ParserConfiguration, TextmateLanguageDefinition } from "./types";
...
export default class TextmateLanguage {
  /** Textmate data. */
  declare textmateData?: TextmateData;
  ...
  constructor(config: TextmateLanguageDefinition) {
    const {
      name,
      alias,
      textmateData,
    ...
    this.textmateData = textmateData;
    this.languageData = {
      ...dataDescription,
      ...convertConfigToLanguageData(this.textmateData?.config || {}),
      ...languageData,
    };
  ...
  load() {
    ...
    if (this.textmateData?.grammar) {
      this.grammar = new Grammar(this.textmateData.grammar);
    }
    ...
    if (this.configure.autocomplete) {
      ...
      this.languageData["autocomplete"] = this.autocompleter.handle.bind(
        this.autocompleter
      );
    }

    // setup language support
    this.language = new Language(facet, new Parser(this));
    this.support = new LanguageSupport(this.language, this.extensions);
    this.loaded = true;

    console.log(
      "LANGUAGE LOADED",
      this.nodeTypes?.map((t) => t.name)
    );

...
import type TextmateLanguage from "../core/language";
import type { AutocompleteHandler } from "../core/types";
import type { ParserNode } from "../grammar/node";
import { nodeTypeProp } from "../parser/constants";
import { TextmateCompletionContext } from "./context";

/** Handles autocompletion for a {@link TextmateLanguage}. */
export class Autocompleter {
  /** A map of node names to completion handlers. */
  handlers = new Map<string, AutocompleteHandler>();

  /** @param language - The {@link TextmateLanguage} to provide completions for. */
  constructor(public language: TextmateLanguage) {
  ...
  /**
   * Gets an autocomplete handler for a {@link ParserNode}.
   *
   * @param type - The {@link ParserNode} to get a handler for.
   */
  private getHandlerFor(type: ParserNode | null | undefined) {
    if (!type) {
      return null;
    }
    ...
    if (!handler && this.config._alsoTypeNames) {
      handler = this.get(type.key);
    }
    if (!handler && this.config._alsoEmitNames) {
  ...
  private traverse(
    ...
    type: ParserNode | null;
    handler: AutocompleteHandler | null;
    traversed: boolean;
  } {
    if (!node) {
      return { node: null, type: null, handler: null, traversed: false };
    }
    ...
    if (!type || !handler) {
      if (this.config._traverseUpwards && node.parent) {
        const {
          ...
          type,
          handler,
        } = this.traverse(node.parent, false);
  ...
  handle(context: CompletionContext) {
    ...
    const mutated = TextmateCompletionContext.mutate(
      context,
      type,
      tree,
      node,

...
import type { GrammarToken } from "../core/types";
import type { GrammarState } from "../grammar/state";
import { perfy } from "../utils/perfy";
import {
  LIMIT_TO_VIEWPORT,
  MARGIN_AFTER,
  MARGIN_BEFORE,
...
export class Parser extends CodeMirrorParser {
  /** A wrapper function that enables mixed parsing. */
  private declare wrapper: ParseWrapper;

  /**
   * @param language - The {@link TextmateLanguage} that this factory
   *   passes to the {@link Parser} instances it constructs.
   */
  constructor(private language: TextmateLanguage) {
    super();
  }
  ...
  createParse(
    ...
    const parser = new Parse(this.language, input, fragments, ranges);
    return parser;
  }
}
...
export class Parse implements PartialParse {
  /** The host language. */
  private declare language: TextmateLanguage;
  ...
  private nextChunk() {
    ...
    while (this.parsedPos < this.region.to) {
      ...
      const match = this.language.grammar!.match(
        this.state,
        str,
        pos - start,
        pos

...
export class ParserNode {
  ...
  /** The key of this node, which may be different to what it emits in the AST. */
  declare key: string;
  ...
  constructor(
    ...
    type = type || createID(autocomplete);
    ...
    const emitType = typeof emit === "string" ? emit : type || "";

    const props: NodePropSource[] = [];

    props.push(nodeTypeProp.add({ [emitType]: this }));

    {
      ...
      if (closedBy) {
        props.push(NodeProp.closedBy.add({ [emitType]: [closedBy].flat() }));
      }
      if (group) {
        props.push(NodeProp.group.add({ [emitType]: [group].flat() }));
      }
      if (fold) {
        props.push(foldNodeProp.add({ [emitType]: parseFold(fold) }));
      }
      if (indent) {
        props.push(indentNodeProp.add({ [emitType]: parseIndent(indent) }));
      }
    }

    this.type = NodeType.define({ id, name: emitType, props });
  }

  /** Special `Node` used for when a rule doesn't emit anything. */
  static None = new ParserNode(-1, { type: "_none", emit: "None" });
}

...
export class Matched {
  ...
  constructor(
    ...
    /** The match's {@link ParserNode} type. */
    public node: ParserNode,
    /** The entire matched string. */
    public total: string,
    /** The start of the match. */
    public from: number,
    /** The children contained by this match's {@link ParserNode}. */
    public captures?: Matched[],
    /**
     * The wrapping mode of the match. There are three modes:
     *
     * - FULL: The {@link ParserNode} in this match contains the entirety of the branch.
     * - BEGIN: The {@link ParserNode} in this match begins the branch.
     * - END: The {@link ParserNode} in this match ends the branch.
     */
   ...
   */
  wrap(node: ParserNode, wrap = this.wrapping) {
   ...
   */
  push(node: ParserNode, side: -1 | 1, wrap = this.wrapping) {
...
function compileLeaf(match: Matched): GrammarToken {
  if (match.wrapping !== Wrapping.FULL && match.node === ParserNode.None) {
  ...
  switch(match.wrapping) {
     case Wrapping.FULL: return [
       match.node === ParserNode.None ? null : match.node.id,
       match.from,

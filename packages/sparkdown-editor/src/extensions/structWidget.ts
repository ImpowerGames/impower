...
const getOrCreateFileInput = (
  ...
  swapButtonEl.onmouseleave = (): void => {
    swapButtonEl.style.backgroundColor = "";
  };
  ...
  plusButtonEl.onmouseleave = (): void => {
    plusButtonEl.style.backgroundColor = "";
  };
...
const updateSoundPreview = (
  ...
  if (structPreview) {
    ...
    state.ctx = canvas.getContext("2d") ?? undefined;
    draw();
    ...
    const onPreviewPointerUp = (): void => {
      ...
      draggableArea.style.cursor = "grab";
      draggableArea.style.backgroundColor = "";
      startX = 0;
      if (clicked) {
        if (state.soundBuffer) {
...
const structDecorations = (view: EditorView): DecorationSet => {
  ...
  const structValidationMap = structTypes.reduce((p, structType) => {
    const validation = getSparkValidation(structType, objectMap);
    if (validation) {
      p[structType] = validation;
    }
    return p;
  }, {} as Record<string, RecursiveValidation>);
  const structRandomizationsMap = structTypes.reduce((p, structType) => {
    const randomization = getSparkRandomizations(structType, objectMap);
    if (randomization) {
      p[structType] = randomization;
    }
    return p;
          ...
          if (type.id === Type.StructColon) {
            ...
            if (struct) {
              ...
              const randomizationOptions: Record<
                string,
                RecursiveRandomization | undefined
              > = {
                default: undefined,
                ...(randomizations || {}),
              };
              const options = Object.entries(randomizationOptions).map(
                ([label, randomization]) => {
                  ...
                  if (structType === "graphic") {
                    ...
                    if (shapes) {
                      graphic.shapes = shapes as Shape[];
                    }

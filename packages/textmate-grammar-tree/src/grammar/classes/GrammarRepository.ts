...
export class GrammarRepository {
   ...
   * @param isRepositoryRule - True if the rule is explicity specified in the repository (as opposed to automatically generated by a capture).
   */
  add(
    obj: ScopedRuleDefinition,
    id?: string,
    isRepositoryRule?: boolean,
  ): ScopedRule;
  add(
    obj: MatchRuleDefinition,
    id?: string,
    isRepositoryRule?: boolean,
  ): MatchRule;
  add(
    obj: SwitchRuleDefinition,
    id?: string,
    isRepositoryRule?: boolean,
  ): SwitchRule;
  add(
  ...
  add(
    ...
    if (id && isRepositoryRule) {
      // Repository rules with no match regex are assumed to match anything
      const matchDef = { match: "(.+)", ...(obj as any) };
      const rule = new MatchRule(this, matchDef);
      this.map.set(rule.id, rule);
      return rule;
    }

...
End the session with a "Skill feedback" section, one entry per problem naming the skill and section, what happened, and the sentence or mechanism you propose. Read the standing inbox body with `gh issue view 510 --json body` and check for a row with the same skill and section covering the same friction. Post each new item as one comment using the intake format in that body and `gh issue comment 510 --body-file <file>`, then read it back with the paginated comments API. A duplicate gets no new intake comment; refer to the existing row or its ticket in the final message.

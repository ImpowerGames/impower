...
The skills under `.claude/skills/` are maintained from what happens when they are followed. Report a command that fails or needs an undocumented flag, a stale path or name, a step whose applicability or reason is unclear, or an ambiguity you had to resolve by guessing.

End the session with a "Skill feedback" section, one entry per problem naming the skill and section, what happened, and the sentence or mechanism you propose. Read the standing inbox body with `gh issue view 510 --json body` and its paginated intake comments. Report each problem your session encounters using that body's intake format and `gh issue comment 510 --body-file <file>`, then read it back. For the same problem, reference its problem ID and reuse your stable runner session or thread reference; each session counts once even if it supplies more observations. Separate problems in the same skill section get separate IDs. Check for your own pending report before posting again. Refer to the problem or its ticket in the final message.

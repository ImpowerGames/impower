...
- A comparison or `switch` on a node typed `GrammarSyntaxNode<SparkdownNodeName>` or `SparkdownSyntaxNodeRef`, including `.name` and `.type.name`. The lowerers under `packages/sparkdown/src/compiler/lower/` receive a `SparkdownSyntaxNodeRef`, whose `node` is typed, so a lookup from `nodeRef.node` through one of the tree helpers is checked; a lookup through the compiler's local child finders, or a walk through lezer's own `firstChild`, `nextSibling` and `parent`, is not.
- A dispatch table keyed by node name declared as `Partial<Record<SparkdownNodeName, …>>`, which the formatter's clause tables and the structural-define tables in the reference annotator and the completion provider are.
...
- A plain `new Set([...])` of names, provided at least one member is a name the grammar has. A set whose members all look like node names but none of which the grammar has is left alone, because that is what a set of another grammar's names or of unrelated CamelCase strings looks like; the cost is that a set whose every member went stale at once is not reported, which is one more reason to build sets with `nodeNameSet`.

`scripts/check-node-names.mjs` covers those. It derives the same legal set as the type (`legalNames` in `scripts/node-names.mjs`), scans every tracked TypeScript file that can reach the tree (one importing a tree helper, `SparkdownNodeName`, `SparkdownSyntaxNodeRef`, `nodeNameSet` or lezer's `SyntaxNode`, or walking from a tree's `topNode`), strips comments and skips string and regex bodies, and reports each literal in one of the positions above that the grammar cannot produce. It also fails when the two generated grammar copies declare different rules, which means one was regenerated without the other; a regeneration forgotten entirely leaves both copies stale and agreeing, and nothing detects that. The Typecheck workflow runs `node --test scripts/check-node-names.test.mjs` and then `npm run check:node-names` before `tsc`. A line that compares a `.name` which is not a node name opts out with a `// not a node name` comment on the line the finding names. The ink engine under `packages/sparkdown/src/inkjs/` compares native-function names and is skipped, as are the grammar-agnostic `textmate-grammar-tree` and `codemirror-vscode-language` packages and one incremental-parse spike that builds its own node set.

Outside both: a name held in a plain string variable, a string built at runtime from a node name, or an object key in a table declared `Record<string, …>`. Declare such values with the union; a table becomes `Partial<Record<SparkdownNodeName, …>>`.
...
| 116 `new Set([...])` constants across 26 files in the compiler, the language server and the extension | node names | Left in place, built with `nodeNameSet(...)` so `tsc` checks them. One set in `lowerSparkdownIfBlock.ts` mixes a literal with computed names and stays a plain set; the script checks its literal. |
| Dispatch tables `SIBLING_CLAUSES` and `MATCHING_IF_BLOCK` in `getDocumentFormattingEdits.ts`, `STRUCTURAL_TYPE_BY_NODE` in `ReferenceAnnotator.ts`, `STRUCTURAL_DEFINE_TYPE` in `getCompletions.ts`, and `IfBlockVariant.prefix` in `lowerSparkdownIfBlock.ts` | node names as keys, values or a prefix | Left in place, declared with the union so `tsc` checks them. |

...
The fingerprint is a hash of the chunk's code and export table, read with every id replaced by what it names: a string's text, a number's value, a symbol's qualified name or, for an anonymous symbol, the ordinal of its part in the statement and its own ordinal in that part. The ids of a session (the chunk's own, and its blocks' sequences) are no part of it. It depends on no table generation and on no order in which a session interned anything, so a statement has the same fingerprint in every session, which is what a durable save compares (section 8). A part of a block statement or of a statement that owns anonymous symbols (a branch's condition, a choice, an alternator, a function written inside the statement) has a fingerprint of its own: the same hash over the part's code alone, with its own anonymous symbols read by their ordinal in the part and without the distance of any jump that leaves the part (a `Choice`'s target, a branch's jump to the end), so that it does not change when another part is written or removed beside it (section 2).
...
A save is JSON, versioned by the engine that wrote it, and owes no compatibility to the current engine's saves, of which none exist outside development. Values go through the existing `JsonSerialisation` round trip, which keeps table identity with `objref` and relinks instances of a define, with one addition. That round trip writes a variable pointer as its name and context index alone (`JsonSerialisation.ts:463-468`), so a closed upvalue loads open and empty and a closure loses what it captured. The save format writes a closed cell once, with its value, under a reference that every closure sharing the cell names, as `objref` does for a table, so a loaded closure keeps its captured value and two closures that shared a cell share it again. Positions and counted symbols are written symbolically (section 8): a frame's address as its symbolic form, a count under its symbol's qualified name or its position, a symbol value by its name.

**Decision:** copied positional state, keyed state marked behind one write barrier, and keyframe and delta images for route search nodes and checkpoints.

**Reason:** a fork and a checkpoint each cost what changed, and neither allocates a copy of the whole state. A continue returns at its newline and takes nothing back (section 6), so an image is the only capture the engine makes.
...
The program is 795 chunks in 57 sequences and 6,443 instructions, with 12 symbols. Its chunks are 61,672 bytes in the prototype's layout, which is smaller than the design's: a three-word header and a three-word block row, and no line table, export table or reference table. The design's seven-word header, five-word block rows and one six-word line table row per statement would add about 32,000 bytes to this program, before its export and reference rows.
...
| #695 | `Native`, `GetVar`, `SetVar`, the jumps, the decision sites, scopes, `Leave`, blocks for `if` and loops, the declaration sequence |

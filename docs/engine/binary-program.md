...
An anonymous symbol belongs to the part of its statement that produced it: a choice, an alternator, a function written inside the statement. The chunk store allocates it when the part is first emitted. When the statement is re-emitted in place (one statement in the changed range before and one after), the old parts are aligned with the new ones, and what belongs to a part goes with it: its anonymous symbols, and the id of the body it heads (section 1). A part whose own code is unchanged, by the part's fingerprint (section 1), keeps what it has wherever it now stands, the nearest in order first where several read the same. The changed parts between two kept ones are paired in order. A new part left without a pair gets new symbols, and an old one left without a pair gives its up. So a choice written above a once-only choice that was taken takes nothing from it, a choice whose text was edited keeps its count, and a function written above another leaves a closure made before the edit calling its own function. The rule's limit is one compile that both inserts a part and edits the part beside it, which pairs the two the wrong way round. Nothing is renumbered by document order, which is what the generated function names do today (`SparkdownCompiler.ts:2062`) and what makes a preview compile rewrite them (#666). A durable save names an anonymous symbol by its statement and its part (section 8).
...
- `addressAt(uri, line)` returns the address of the beat or statement on that line, or nothing. It finds the flow whose line range holds the line, then descends: a binary search of the sequence's line starts gives the statement, and when the line falls inside one of that statement's bodies (each body's first line and span follow from the owner's block rows and the root's rows, section 1) it goes on into that body's sequence, and otherwise reads the chunk's line table. A beat's address is the offset of its `LineStart`, so the cue line, the directive and the dialogue text of one beat return one address, and the lines of two beats return two.
- `locationOf(address)` returns the script and range of the line table row that covers the offset: the row's anchor resolved, the statement's line start in its sequence added, and then, for each owner up to the flow, where that body starts in its owner and the owner's own line start.
...
| `sequence(id)` | this root's row for the sequence: its chunks, their line starts, the lines it spans, its owner's chunk id, its block index and its flow, or nothing for a body the program no longer has. A block's id comes from its owner's block table |
...
The scripts have changed since that sitting in two ways that touch timed code, and in none that touches the symbol or order modes: a block row of the prototype's chunks is one word longer, which adds no read to a step, and the look-ahead's restorable state also saves the lengths of its error and warning arrays, while its restore walks an empty table log. Every count in this section (chunks, bytes, symbols, statements, lines) is what the scripts as they are report. A sitting on the scripts as they are was taken at 67 to 96 percent load and is not tabulated: both sides of every comparison ran 2 to 5 times slower (the scene 51 to 198 ms on the engine against 3.5 to 9.0 on the prototype, a look-ahead pair 6.8 to 13.7 microseconds against 0.8 to 2.1), which keeps the engine 6 to 22 times the prototype, and the few nanoseconds the symbol mode looks for were lost in the spread, with opposite signs on the two projects.

...
- Listing: the fingerprints of a sequence's entries in order, as a save writes them (section 8).
...
| 5 | chunk id |
| 6 | fingerprint of the statement's source, for the save format |
...
The fingerprint is a 32-bit hash of the statement's source and of nothing else: the text of a plain statement, and the kind alone of a block statement, with comments removed and runs of whitespace collapsed. It is what a durable save compares (section 8) and what aligns the parts of a re-emitted statement (section 2), so it is a fact about what the author wrote and not about what the writer emitted: it holds across sessions, across a reseed, across a release of the compiler that lowers the statement differently, and across a change to the value of a constant the statement reads, none of which a save should notice. A block statement hashes as its kind because its own parts (a condition, a choice's line) are what an author edits most and the statements of its bodies are matched on their own, which is Ren'Py's rule for `if` and `menu` (`renpy/ast.py`, `diff_info`). A part of a block statement or of a statement that owns anonymous symbols (a branch's condition, a choice, an alternator, a function written inside the statement) has a fingerprint of its own, the same hash over the part's source alone, so that it does not change when another part is written or removed beside it (section 2).
...
The code before the `Done` that ends the presentation reads state and writes only the captured texts: no `Visit` and no assignment of the writer's own stands there. So a `choose` statement can be run again from its start to raise the same choices, which is how a save taken at the choices resumes (section 8): it holds neither the choices nor their threads.
...
A save is the store's newest keyframe and the deltas since it, each beat with its positions in the durable form of section 8, so it holds the last `baseInterval` beats the player passed and the loader can step back through them. That costs a handful of small deltas beside the keyframe, which is the size a save has in any case. Values go through the existing `JsonSerialisation` round trip, which keeps table identity with `objref` and relinks instances of a define, with one addition. That round trip writes a variable pointer as its name and context index alone (`JsonSerialisation.ts:463-468`), so a closed upvalue loads open and empty and a closure loses what it captured. The save format writes a closed cell once, with its value, under a reference that every closure sharing the cell names, as `objref` does for a table, so a loaded closure keeps its captured value and two closures that shared a cell share it again. Positions and counted symbols are written in the durable form (section 8): a frame's position as the chain of sequences that holds it, each with its listing, and an anchor in its statement; a count under its symbol's qualified name or its position; a symbol value by its name. A beat that waits at choices is written at the start of its `choose` statement and holds neither the choices nor their threads, because running the statement again raises them (section 4). The loader takes a save of an older format through a migration per version and refuses a newer one, naming the version; a fixture save of each format version that shipped stays in the tests.
...
**Decision:** copied positional state, journaled keyed state behind one write barrier, a single-level save point for the look-ahead that also reaches the simulator's cursors and the three snapshot hooks, keyframe and delta images for route search nodes and checkpoints, and a save that is the newest keyframe and the beats since it, each beat's positions in the durable form.

**Reason:** the look-ahead, a fork and a checkpoint each cost what changed, none allocates a copy of the whole state, and a save gains the beats to fall back through at the price of the deltas the store already holds.
...
An address that outlives a session, or a chunk, is written as a place in the source and never as a place in the code, because the code of a statement changes whenever the statement or the compiler does and a save should notice neither. The written form has three parts.

The chain: the sequence that holds the position and each sequence that holds that one, up to the flow, one level per sequence. A level is written as the sequence's listing, which is the fingerprints of its entries in order (the whole sequence up to 512 entries, and beyond that 256 on each side of the ordinal), and the ordinal of the entry the position is in. The flow's level carries the flow's qualified name, and a body's level carries the fingerprint and the ordinal of the part of the owner that heads the body (the branch's condition, the choice's line, the `then` label), which is what places the body when parts were written or removed beside it (section 2). A listing is written once per sequence and shared by every frame and every beat of the save that names the sequence, so the largest scene of the Raffles and Bunny project costs a save a few kilobytes of them.

The statement: its fingerprint (section 1).

The anchor: where in the statement the position is, named by the statement's structure and never by an offset. It is the statement's start; or after its j-th call, counted over the `Call`, `CallVar` and `CallValue` instructions of its chunk, for a frame whose return site is inside an expression; or after its k-th body, or at that body's break, for a position the owner resumes at when a body ends. The loader turns an anchor back into an offset from the chunk the program has now: the entry code, the j-th call, or the k-th block row's resume or break offset. A beat that waits at choices is anchored at the start of its `choose` statement, so that the loader runs the statement's presentation again and the player sees the choices the new program has, as Ren'Py resumes at the start of a `menu`; the choices and their threads are not written (section 4).
...
1. The flow, by its qualified name. When the program has no flow of that name, every sequence of the program is searched for the run of fingerprints around the position, eight on each side, and the sequence with the longest matching run is taken when one sequence beats the rest, which is how a save inside a renamed scene loads; otherwise the position is unplaced.
2. Each level, by aligning the saved listing with the sequence the program has now, as Ren'Py aligns the statements of an old and a new script (`renpy/script.py`, `merge_names`): the matching blocks of the two lists are found and the saved ordinal is carried across by the block it falls in. That is what finds a statement again after an insertion above it, after an edit to the statement itself, after edits to its neighbours, and after a block of statements was moved within the flow. A body's level is placed by aligning the owner's parts and taking the body of the matched part. An ordinal that falls in no matching block, because its statement is gone, leaves the position unplaced.
3. The anchor, from the placed statement's chunk. A call anchor whose call the chunk no longer has returns the frame to the statement after its caller, with the eval stack cut back to the frame's height and a warning, which is Ren'Py's rule for a return site that is gone (`renpy/execution.py`, the return stack in `run`); a body anchor whose body is gone leaves the position unplaced.
4. The beat. A beat is placed when every frame of every thread of its position is. The loader takes the newest beat of the save that is placed, restores its image (section 7) and resumes at its position. A placed beat older than the newest carries its own variable values, counts and tables, so nothing runs twice and nothing runs in a state it did not run in. This is Ren'Py's load (`renpy/rollback.py`, `rollback_core`), which pops its log until it reaches a statement the script still has.
5. No beat is placed: the load is refused and names the flow of the newest beat. The engine does not guess a position it cannot name, which is inkcpp's rule as well (`inkcpp/story_impl.cpp`, `new_runner_from_snapshot`).

What no saved form tells apart is a run of identical statements, and Ren'Py's cannot either: the alignment keeps order, so a save taken at the second of two identical lines, loaded after a third identical line was written between them, resumes a line early, and one taken at the second of three, loaded after the first was deleted, resumes a line late. Either way what a player can see is one identical line more or fewer, and a statement with an effect runs once more or once less. Telling such statements apart needs an identity kept in the source, and the design keeps none. Within a session the case does not arise, because a position whose chunk the new root still holds is not translated at all.

Within a session, when a root arrives, a checkpoint's address whose chunk the new root does not hold is translated once: the old root gives its saved form, listings included, and the new root places it by the same steps.

**Decision:** an opaque address of chunk id and offset for the running program, two lookups on the root in place of the path-location table, and a saved form of listings, fingerprints of the source and anchors in the statement's structure, which the loader aligns with the program it has and falls back through the beats the save holds.
...
The program is 795 chunks in 57 sequences and 6,443 instructions, with 12 symbols, and the prototype opened 674 look-aheads, one per line. Its chunks are 61,672 bytes in the prototype's layout, which is smaller than the design's: a three-word header and a three-word block row, and no line table, export table or reference table. The design's seven-word header, five-word block rows and one six-word line table row per statement would add about 32,000 bytes to this program, before its export and reference rows.
...
| #699 | the write barrier, images, checkpoints, the save format with its beats and listings, the loader's alignment and its fall back |

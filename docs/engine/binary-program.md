...
- Layout hash: a hash of a chunk's code, by which a save knows whether a frame saved inside the statement's code can resume there (section 1).
...
The chain: the sequence that holds the position and each sequence that holds that one, up to the flow, one level per sequence. A level is written as a window of the sequence's listing, the fingerprints of its entries in order from a first ordinal the window names, and the ordinal of the entry the position is in. A window covers the whole sequence up to 512 entries and otherwise 256 entries on each side of the position; the windows that the frames and beats of one save cut from one sequence are merged where they overlap and written once each, so every position keeps its own neighbours whatever else the save names in that sequence. The flow's level carries the flow's qualified name. A body's level carries the owner's part listing, the fingerprints of the owner's own parts in order (its branches' conditions and `else`, its choices' lines, its `then` label), and the ordinal of the part that heads the body, which is what the three passes of section 2 need to place the body when parts were written, edited, reordered or removed beside it. The largest scene of the Raffles and Bunny project costs a save a few kilobytes of listings.

The statement: its fingerprint (section 1).

The anchor: where in the statement the position is. It is one of four. The statement's start. The end of the sequence, written as an ordinal equal to the number of entries, for a flow that has run out and for a body whose owner is about to resume. After the body headed by a part, or at that body's break, the part named by its fingerprint and its ordinal in the owner's part listing, for a position the owner resumes at when a body ends. Or inside the statement's code, written as the offset beside the chunk's layout hash (section 1), for a frame whose return site is inside an expression, for a thread whose fork is still running (the original resumes at its `Thread`'s offset) and for anything else that is neither a statement boundary nor a body's end. The loader turns the first three back into offsets from the chunk the program has now (the entry code, the sequence's end, the matched part's block row), and takes the fourth as it is when the layout hash of the chunk the program has now equals the saved one, which says that the code, and so the offset, the temporaries and the eval stack the frame holds, are what they were; otherwise the frame is placed after its statement (step 4 below).

A choice waiting to be chosen is not written. A beat at which choices are waiting is written as the position and state the continue that raised them started from: the beat before it, or the initial image (section 7). Loading that beat and continuing raises the choices the new program has, from every thread that contributes one, from a state that predates their conditions' effects, so those run once; that is how Ren'Py resumes at the start of a `menu`. A checkpoint taken after a choice was made is the same with the chosen choice added, named by its `Choice` instruction's statement in the saved form and by the fingerprint and ordinal of its part; loading runs the continue again and chooses the part that section 2's alignment matches, and is unplaced when none is.

An anonymous symbol is written as the saved form of its statement with, in place of the anchor, the fingerprint of the part it belongs to and that part's ordinal among the statement's parts, beside the statement's part listing; the loader aligns the saved parts with the statement's parts as a re-emit does (section 2), so a choice's count goes to the choice with that fingerprint, the nearest ordinal first, and a part it cannot place starts fresh. A count under a qualified name loads under that name, after the names a placed flow renames (step 3 below) are rewritten.

Loading places each position of a beat, then chooses the beat:

1. The flow, by its qualified name, when the program has one.
2. Each level, by aligning the saved window with the sequence the program has now in the three passes that align a re-emitted statement's parts (section 2). First the matching blocks of the two lists, in order, as Ren'Py aligns the statements of an old and a new script (`renpy/script.py`, `merge_names`). Then, among the entries left on both sides, equal fingerprints wherever they now stand, nearest first, which is how a run of statements moved within the flow is found. Then, in each run of entries left between two matched ones, or between a matched one and either end, the old paired with the new in order, which is how a statement edited in place is found by its unchanged neighbours. The saved ordinal takes the entry it was matched to. A body's level is placed by aligning the owner's saved part listing with the owner's parts the same way and taking the body of the matched part. A saved entry left without a match, because its statement or the part that headed its body is gone, leaves the level unplaced.
3. A level that step 1 or 2 leaves unplaced is looked for in every sequence of the program by the run of fingerprints around the position in its window, eight on each side. It is placed in the sequence whose longest matching run is unique among sequences, covers the statement and at least two of its neighbours (or the whole window when the window is shorter), and beats every other sequence's; the chain above it is then the one the root holds for that sequence, and the saved levels above are not used. That is how a save inside a renamed scene, one inside a body moved under another owner, and one inside a block of statements moved to another flow load. A tie, or a shorter run, leaves the position unplaced. A flow placed this way under another qualified name has the old name rewritten to the new one in every count key, symbol value and anonymous-symbol form of the save that carries it, so the counts and the values that drove the flow before the rename drive it after; a label that was itself renamed keeps nothing, because its statement's fingerprint changed with its name, and the count under its old name is dropped.
4. The anchor, from the placed statement's chunk, as above. A frame whose position was inside the statement's code under a layout hash the chunk no longer has is placed after the statement instead: its callee frames, if any, run to their return and the value they return is dropped; the frame resumes at the next entry of its sequence, or at the owner's resume offset when the statement was the last, with its eval stack cut to the height at which the frame itself was pushed, which is its height at any statement boundary of that frame, its open captures dropped, its scopes cut to the depth the block stack gives that position (section 1) with the upvalues bound in the dropped scopes closed as `EndScope` closes them, and a warning that names the statement. That is Ren'Py's rule for a return site that is gone (`renpy/execution.py`, the return stack in `run`), which sends the return to the statement after the caller. A body position under a loop is placed after the loop the same way when the loop statement's layout hash differs, because the frame holds the loop's hidden temporaries under the names that layout gave them (section 10); a body under an `if` or a `choose` needs no such check, because those owners declare nothing the frame holds. A placement after a statement is a placement with a warning, not an exact one.
5. The beat. A beat is placed when every frame of every thread of its position is, exactly or after a statement. The loader takes the newest beat whose frames are all placed exactly, and when there is none the newest beat that is placed at all; it restores that beat's image (section 7) and resumes at its position, which for the beat before a menu means running the continue that raises the menu. A placed beat older than the newest carries its own variable values, counts and tables, so nothing runs in a state it did not run in. This is Ren'Py's load (`renpy/rollback.py`, `rollback_core`), which pops its log until it reaches a statement the script still has.
6. No beat is placed: the load is refused and names the flow of the newest beat. The engine does not guess a position it cannot name, which is inkcpp's rule as well (`inkcpp/story_impl.cpp`, `new_runner_from_snapshot`).

What no saved form tells apart is a run of identical statements, and Ren'Py's cannot either: the alignment keeps order, so a save taken at the second of two identical lines, loaded after a third identical line was written between them, resumes a line early, and one taken at the second of three, loaded after the first was deleted, resumes a line late. What a player can see is one identical line more or fewer, and a statement with an effect runs once more or once less; it is the one case in which a placed beat runs something in a state it did not run in. Telling such statements apart needs an identity kept in the source, and the design keeps none. A statement split in two, or merged with its neighbour, is an edited statement to the alignment: the save is placed at whichever new statement the third pass pairs with it, at its start. Within a session none of this arises, because a position whose chunk the new root still holds is not translated at all.
...
**Decision:** an opaque address of chunk id and offset for the running program, two lookups on the root in place of the path-location table, and a saved form of listing windows, part listings, fingerprints of the source and anchors in the statement's structure, with an offset only beside the layout hash that validates it, which the loader aligns in three passes with the program it has, searches every sequence for when a level is gone, and falls back through the beats the save holds.
...
| #697 | `Choice`, the `choose` chunk, anonymous count symbols, choices in a thread, a presentation that runs again from the statement's start |
| #698 | `Sym` and symbol values, `Call`, `CallVar`, `CallValue`, `Return`, `Pack`, `Unpack`, `VarPtr`, function-body blocks, the builtin surface |
| #699 | the write barrier, images taken at the newline, checkpoints, the save format with its beats, windows and part listings, the saved form, the loader's alignment, search and fall back |

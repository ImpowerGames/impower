...
A checkpoint restore waits for the images and fonts the checkpoint displays, bounded by `restore_timeout`. Preview has no running clock, so this is how preview avoids showing a restored backdrop late, and the same gate covers what the preview is about to write: the cursor's beat and the beats after it in the same script with no located line between them, up to six in all, because image-only lines display together with the first line of text below them, across any number of blank lines, while a line of dialogue between two of them displays on its own and ends the preview there. The gate applies when the cursor is on a beat, or before the scene's first beat, which then starts it. A cursor on a line between beats is gated on nothing: the preview writes that line and nothing that names an asset, and what an earlier beat showed is in the checkpoint, which this gate covers on its own. Nothing is gated for a path the program no longer knows (a remembered preview point the last edit removed) or one inside a function, which a preview cannot start in. The preview then writes that beat the moment the game is connected, with its line and its portrait resident together.

...
A checkpoint restore waits for the images and fonts the checkpoint displays, bounded by `restore_timeout`. Preview has no running clock, so this is how preview avoids showing a restored backdrop late, and the same gate covers what the preview is about to write, and it learns that by running the beat ahead of its display: at connect the engine steps the story to the beat's flush exactly as the preview would, from the loaded checkpoint or, when the route to the cursor failed, from a reset story, keeps the flushed instructions instead of building the coordinator that displays them, gates the images they show, and the preview then displays those instructions instead of running the beat again. The beat runs once, so whatever it does (a variable it changes, an error it raises, a scene it enters) happens once, and nothing a completed run does is undone; only a preview that turns out to be for another path than the beat ran for puts the game back (from a save taken before a run that continued from a checkpoint; the other branches reset the story before they run), and starting, resetting, reloading, or recompiling drops the run. What the run tells the page (an error it raises, what it executed) is told when the run displays, in the order it happened, so the page hears about the beat once, when it is shown, as it would of a beat the preview ran itself, and hears nothing of a run that is dropped; the execution report names what the beat ran and the conditions it met, not what the layouts' bindings evaluate when they mount after the run. The run is armed by the page's mark alone: the connect runs the beat for a point marked since the last preview, and a host that connects the same game again without marking gets a connect that runs nothing. Nothing is kept of a run that throws or stops short of its flush (at a breakpoint inside the beat, or as a runaway): a stopped run leaves the story part-way through a line, which the layouts mounted next could not evaluate, so its game is put back first, from the same save or by rewinding the story, and the preview runs the beat itself and meets the same stop, or throws the same error, after the game is connected; a run kept for a point the preview cannot resolve, or one the game is started with, is put back the same way. What the run changes in the story's state is not deferred, and the layouts mount after the run: a binding that reads a variable the beat changes shows the beat's value from the connect, while the beat's own writes wait for the gate, and a dropped run's values until the preview runs the beat itself. What displays together is the story's decision as it runs (a `[[hide]]` or a conditional between two image lines displays nothing and the two show together, a line of dialogue between them ends the beat, a divert carries the beat into the next scene's first beat, a beat can name any number of pictures), so nothing read off the source can say it, and the beat itself can. The checkpoint's own pictures are requested before the beat runs, so their loads are under way meanwhile, except the pictures on targets the connect clears (a portrait shows only with the beat that writes it), which the restore does not write and the gate does not wait for; and the interpreter's own prefetch of the names it parses is skipped for a beat running ahead, since the gate asks for them next. Nothing runs for a path the program no longer knows (a remembered preview point the last edit removed) or one inside a function, which a preview cannot start in. The preview then writes that beat the moment the game is connected, with its line and its portrait resident together.

...
A checkpoint restore waits for the images and fonts the checkpoint displays, bounded by `restore_timeout`, except the pictures on targets the connect clears (a portrait shows only with the beat that writes it), which the restore does not write and the gate does not wait for. Preview has no running clock, so this is how preview avoids showing a restored backdrop late. The beat under the cursor has a gate of its own, at preview: the preview runs the story to the beat's flush exactly as play does, from the loaded checkpoint or, when the route to the cursor failed, from a reset story, holds the flushed instructions back instead of building the coordinator that displays them, waits until the pictures they show are resident (priority 0, a pin of its own, `preview:<n>`, bounded by `restore_timeout`, after which the beat displays anyway), and then displays them, so the line and its portrait land together; a beat with no picture, or whose pictures are already resident, displays at once. The player's worker waits for the preview to settle before it closes the reconcile pass, which is when the page sweeps the elements the previous preview left, so a write that lands after the wait is never swept. What displays together is the story's decision as it runs (a `[[hide]]` or a conditional between two image lines displays nothing and the two show together, a line of dialogue between them ends the beat, a divert carries the beat into the next scene's first beat, a beat can name any number of pictures), so nothing read off the source can say it, and the beat itself can. The layouts' bindings refresh when the beat displays, as they do in play, so nothing the beat changed shows before its line does; a handler that runs while the beat waits repaints the layouts from the story's state, which the beat has already changed, as it does in play when a handler runs while a beat's own gate waits. The choices the last displayed beat presented leave before the wait, after the beat's pictures have been asked for, so none of them can be clicked while it waits, and a run that stops short takes them away too; the game remembers the targets the beat presented (a route replay presents none), so a beat that presented none costs the page nothing. On the Game Preview's path the connect takes them: it clears every transient target out of the page and out of the module's state and forgets them for the game, so a preview after a connect sends nothing for them on either route, and the preview's own clear is for a host that previews without a connect between. The execution report is what the run executed, taken when it stopped, so nothing a handler runs during the wait is in it. The interpreter's own prefetch of the names it parses is skipped while the flush is held, since the gate asks for them next. A run that stops short of its flush (at a breakpoint inside the beat, or as a runaway) displays nothing, as in play. A preview that starts while another waits takes over, whether or not its point resolves, and so does a load, a recompile, a reset, a start, a connect, a debug step, or the game's destruction: the earlier preview's wait ends at once and it displays nothing, its point previews again when asked, and its pin goes with the next beat's gate, right after that gate's request, or when its own load settles, or with the game, whichever is first, so a picture still queued for the beat the cursor has left leaves the express lane once the beat the cursor is on has asked for its own, while a load already in flight finishes there; a release before that request would let the page's background queue, which pauses while a gate is pending, start loads beside the picture the next beat waits on, so the queue stays paused between the two, and the connect's own gates (the layouts' fonts, the restore) settle before that request, so they do not release it. When nothing waits (the next beat needs no gate, the take-over's point resolves to none, a debug step, whose beat displays at once, play begins with a beat that needs no picture) the pin goes at once and the queue resumes. A repeat of the point whose preview is waiting settles with it. With `restore_timeout` at 0 the wait is unbounded, as every gate's is: the beat displays when the page answers or the next update takes the preview over. `preview()` resolves to the path previewed, and to null both for a point that resolves to none and for a preview taken over while it waited. The worker's display stands down likewise when a newer display, or PLAY, overtakes it while it waits, so the elements the newer display's reconcile pass adopted are swept by that display alone. Nothing runs for a point the program no longer knows (a remembered preview point the last edit removed); a point inside a function runs the function's body from its start, which runs out of content before it shows a picture, and the gate follows what that writes.

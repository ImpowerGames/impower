...
test("speech still runs when keyboard fails; partial failure is visible", async () => {
  ...
  const result = await deliver(
    { message: "Hey, I finished the itinerary. Can you check the dates?" },
    {
  ...
  assert.equal(
    spoken,
    "Hey, I finished the itinerary. Can you check the dates?",
  );
...
test("device lease prevents overlap and releases on error", async () => {
  ...
  assert.equal(
    await withDeviceLease(async () => "released", options),
    "released",
  );
});
test("real MCP client discovers and invokes tool without hardware effects", async () => {
  ...
  try {
    ...
    assert.deepEqual(Object.keys(notifyTool.inputSchema.properties), [
      "session",
      "message",
      "category",
    ]);
    ...
    const ack = await client.callTool({
      ...
      arguments: { notificationId: "11111111-1111-4111-8111-111111111111" },
    });
    assert.equal(JSON.parse(ack.content[0].text).dryRun, true);
    const badAck = await client.callTool({
      name: "acknowledge_notification",
      arguments: { notificationId: "guessed" },
    });
    assert.equal(badAck.isError, true);
    const message =
      "Hey, I finished organizing the photos. Can you pick a cover?";
    ...
    assert.deepEqual(payload.lighting.handlers[0].color, {
      red: 0,
      green: 255,
      blue: 80,
    });
    const needsInput = await client.callTool({
      name: "notify_user",
      arguments: { message, category: "user_input_needed" },
    });
    assert.deepEqual(
      JSON.parse(needsInput.content[0].text).lighting.handlers[0].color,
      { red: 255, green: 140, blue: 0 },
    );
    const badCategory = await client.callTool({
      name: "notify_user",
      arguments: { message, category: "merge_ready" },
    });
    assert.equal(badCategory.isError, true);
    const invalid = await client.callTool({
      name: "notify_user",
      arguments: { message: "a".repeat(281) },
    });
...
test("generic categories route Codex to F1 and Claude to F2", async () => {
  ...
  for (const [app, key] of [
    ...
    for (const [category, color] of Object.entries(config.colors)) {
      ...
      assert.deepEqual(handler.color, {
        red: color[0],
        green: color[1],
        blue: color[2],
      });
  ...
  assert.equal(
    binding(config, "done", "other").handlers[0].zone,
    "function-keys",
  );
});
...
test("CLI category option preserves the spoken sentence and rejects missing values", async () => {
  ...
  const { stdout } = await run(
    process.execPath,
    [main, "notify", message, "--category", "user_input_needed", "--dry-run"],
    { env: { ...process.env, AGENT_ALERT_APP: "claude" } },
  );
  const result = JSON.parse(stdout);
  assert.deepEqual(result.alert, { message, category: "user_input_needed" });
  assert.deepEqual(result.lighting.handlers[0]["custom-zone-keys"], [59]);
  await assert.rejects(
    run(process.execPath, [main, "notify", message, "--category", "--dry-run"]),
  );
});

...
export const alertShape = {
  session: z
    ...
    .optional()
    .describe(
      "Optional verified desktop session ID for the shortcut. Omit when unavailable.",
    ),
  message: z
    .string()
    .trim()
    ...
    .max(280)
    .describe(
      "A short notification accompanying the normal chat handoff: what you finished and, if needed, what the user should do next. Any topic. Use plain text without markdown, code, or secrets; the receiver chooses how to deliver it.",
    ),
  category: z
    .enum(["done", "user_input_needed", "blocked"])
    .default("done")
    .describe(
      "done: finished with no user action required; user_input_needed: a normal question, decision or review; blocked: a problem requires user help before work can continue.",
    ),
};
...
const configSchema = z
  .object({
    ...
    shortcuts: z
      ...
      .strict()
      .refine(
        (value) => value.codex !== value.claude,
        "Choose different shortcut keys for each app.",
      ),
  })
  .strict();
export async function readConfig() {
  ...
  try {
    local = JSON.parse(
      await readFile(
        process.env.AGENT_ALERT_CONFIG ||
  ...
  const colors = legacyColor
    ? {
        done: legacyColor,
        user_input_needed: legacyColor,
        blocked: legacyColor,
      }
    : defaults.colors;
  return configSchema.parse({
    ...defaults,
    ...overrides,
    colors: { ...colors, ...local.colors },
    keys: { ...defaults.keys, ...local.keys },
    shortcuts: { ...defaults.shortcuts, ...local.shortcuts },
  });
}
...
export function binding(
  config,
  category = "done",
  app = process.env.AGENT_ALERT_APP,
) {
    ...
    handlers: [
      {
        ...
        ...(keys ? { "custom-zone-keys": keys } : { zone: config.zone }),
        mode: "color",
  ...
  for (const dir of ["SteelSeries Engine 3", "GG"]) {
    try {
      const props = JSON.parse(
        await readFile(
          `${process.env.ProgramData || "C:/ProgramData"}/SteelSeries/${dir}/coreProps.json`,
          "utf8",
        ),
      );
      if (/^127\.0\.0\.1:\d+$/.test(props.address))
        return `http://${props.address}`;
    } catch (e) {
      if (e.code !== "ENOENT") throw e;
    }
  }
  throw new Error(
    "SteelSeries Engine address not found. Start SteelSeries GG / Engine.",
  );
}
async function lighting(alert, config) {
  ...
  const post = async (route, body) => {
    ...
    if (!response.ok)
      throw new Error(
        `SteelSeries ${route}: ${response.status} ${await response.text()}`,
      );
  };
  await post("game_metadata", {
    game,
    game_display_name: "Agent Notification Alerts",
    developer: "Local agent tools",
  });
  try {
    await post("bind_game_event", binding(config, alert.category));
    const end = Date.now() + config.durationMs;
    while (Date.now() < end) {
      await post("game_event", {
        game,
        event: "ATTENTION",
        data: { value: 1 },
      });
      await new Promise((resolve) =>
        setTimeout(resolve, Math.min(1000, Math.max(0, end - Date.now()))),
      );
    }
  } finally {
  ...
  return new Promise((resolve, reject) => {
    const child = spawn(
      ...
      [
        ...
        "-NonInteractive",
        "-File",
        fileURLToPath(new URL("./speak.ps1", import.meta.url)),
      ],
      {
        windowsHide: true,
...
export async function deliver(
  input,
  { dryRun = false, config, light = lighting, speech = speak } = {},
) {
  ...
  if (dryRun)
    return {
      dryRun: true,
      alert,
      lighting: binding(config, alert.category),
      speech: config.speech,
    };
  const channels = [
    ["keyboard", config.keyboard, light],
    ["speech", config.speech, speech],
  ];
  const results = await Promise.all(
    channels.map(async ([name, enabled, fn]) => {
      if (!enabled) return [name, { status: "disabled" }];
      try {
        return [name, { status: "ok", detail: await fn(alert, config) }];
      } catch (e) {
...
export async function withDeviceLease(
  action,
  { port = 39761, waitMs = 120000 } = {},
) {
  ...
  while (true) {
    ...
    try {
      ...
      if (Date.now() >= deadline)
        throw new Error(
          "Notification devices busy. Another alert or application holds local port 39761.",
        );

...
export function validateReviewPlan(input) {
  ...
  for(const review of plan.reviews) {
    ...
    if(review.transport==='native-codex-jsonl') {
      ...
      const root=fs.realpathSync(review.permissions.cwd);
...
export async function createReviewJob(input,host,{verifyExecutable=verifyReviewerExecutable}={}) {
...
export function launchSupervisor(dir) {
  ...
  try{const child=spawn(process.execPath,[here,'run',dir],{cwd:dir,detached:true,windowsHide:true,stdio:['ignore',log,log]});child.on('error',error=>console.error(`Monitor launch failed: ${error.message}`));child.unref();return{pid:child.pid};}finally{fs.closeSync(log);}
}
...
export async function advanceReviewJob(dir,host,{identify=processIdentity,failpoint=()=>{}}={}) {
  ...
  if(!submissionOutstanding(rows)) {
    ...
    if(state.state==='disconnected'||state.state==='unknown') {
      if(state.reason)await transaction(dir,current=>{if(last(current,'destination-unavailable')?.reason!==state.reason)appendEvent(dir,'destination-unavailable',{reason:state.reason});});
  ...
  if(submissionOutstanding(rows)&&!last(rows,'continuation-accepted')) {
    let accepted;try{accepted=await host.reconcile(envelope,plan);}catch(error){await transaction(dir,current=>{if(last(current,'reconciliation-unavailable')?.reason!==error.message)appendEvent(dir,'reconciliation-unavailable',{reason:error.message});});return jobStatus(dir);}

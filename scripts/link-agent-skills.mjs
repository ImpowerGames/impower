...
export function linkAgentSkills(root = repositoryRoot, { repairLinks = false, reportPlan = () => {} } = {}) {
  ...
  const plans = toolDirectories.map((tool) => {
    ...
    if (stat?.isSymbolicLink()) {
      let target;
      try { target = fs.realpathSync(destination); }
      catch (error) { if (error.code !== "ENOENT") throw new Error(`Cannot resolve link ${destination}: ${error.message}; no links were changed`); }
      if (target !== source) {
        ...
        return { destination, action: "repair", previousTarget: fs.readlinkSync(destination) };
      }
  ...
  // Emit recovery information before any unlink, including when a later create
  // fails. The CLI persists these records in its captured stdout.
  reportPlan(plans.map((plan) => ({ ...plan, target: source })));
  ...
  return plans.map((plan) => ({ ...plan, target: fs.realpathSync(plan.destination), kind: process.platform === "win32" ? "junction" : "symlink" }));
}
...
if (process.argv[1] && fs.realpathSync(process.argv[1]) === fileURLToPath(import.meta.url)) {
  try {
    ...
    console.log(JSON.stringify(linkAgentSkills(repositoryRoot, { repairLinks: process.argv.includes("--repair-links"), reportPlan: (plans) => console.log(JSON.stringify({ event: "planned", plans })) }), null, 2));
  }

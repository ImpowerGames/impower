...
export const searchLineState = StateField.define<SearchLineState>({
  create() {
    return { search: "" };
  },
  update(value, tr) {
...
export const getSearchLineQuery = (state: EditorState): SearchLineQuery => {
  const query = state.field(searchLineState, false);
  return query || { search: "" };
};

export const openSearchLinePanel: Command = (view) => {
  const state = view.state.field(searchLineState, false);
  if (state && state.open) {
    return false;
  }
...
export const searchLinePanel = (config?: SearchLineConfig): Extension => {
  const searchExtensions = [searchLineState, keymap.of(searchLineKeymap)];
  return config

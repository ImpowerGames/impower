...
export const doRestructure = async (
  ...
  console.log("Copy all pitched_games/{id}/contributions/{contributionId} to pitched_projects/{id}/contributions/{contributionId}");
  ...
  await Promise.all(userSnaps.docs.map(async (s) => {
    const submissionsColRef = firestore.collection("users").doc(s.id).collection("submissions");
    const newSubmissionDoc = {_createdBy: s.id, _documentType: "PathDocument", _updates: {}};
    ...
    if (!commentsSnap?.data()?._updates) {
      promises.push(bulkWriter.set(commentsDocRef, newSubmissionDoc, {merge: true}));
    }
    ...
    if (!projectsSnap?.data()?._updates) {
      promises.push(bulkWriter.set(projectsDocRef, newSubmissionDoc, {merge: true}));
    }

    const studiosDocRef = submissionsColRef.doc("studios");
    const studiosSnap = await studiosDocRef.get();
    if (!studiosSnap?.data()?._updates) {
      promises.push(bulkWriter.set(studiosDocRef, newSubmissionDoc, {merge: true}));
    }
    ...
    if (!suggestionsSnap?.data()?._updates) {
      promises.push(bulkWriter.set(suggestionsDocRef, newSubmissionDoc, {merge: true}));
    }

...
export const doCleanup = async (
  ...
  contributionSnaps.forEach((s) => {
    if (s.exists) {
      bulkWriter.delete(firestore.collection("pitched_games").doc(s.ref.parent.parent.id).collection("contributions").doc(s.id), s.data());
    }
  })  
  ...
  deletedSubmissionSnaps.forEach((s) => {
    if (s.exists) {
      bulkWriter.delete(firestore.collection("users").doc(s.ref.parent.parent.id).collection("deleted_submissions").doc(s.id.replace("project", "game")));
    }
  })  
  // Update all tags
  ...
  userSnaps.forEach((s) => {
    if (s.exists) {
      bulkWriter.delete(firestore.collection("users").doc(s.id).collection("submissions").doc("games"));
    }
  })
  ...
  Object.entries(usersVal).forEach(([userId, userData]) => {
    Object.entries(userData.agg || {}).forEach(([type, agg]) => {
      ...
      userData.agg[type].count = Object.keys(userData.agg[type].data || {}).length;
